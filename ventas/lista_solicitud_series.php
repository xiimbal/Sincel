<?php
    ...
    $idSolicitud = $_GET['id'];
    $permisos_grid->tienePermisoEspecial($_SESSION['idUsuario'], 35)
    ...
    $consulta = "SELECT b.id_bitacora,b.NoParte,TRIM(b.NoSerie) AS NoSerie,kb.ClaveCentroCosto,
        ...
        FROM k_solicitudbitacora AS kb
        LEFT JOIN `c_bitacora` AS b ON kb.id_bitacora = b.id_bitacora
        ...
        WHERE kb.id_solicitud = $idSolicitud ORDER BY NoParte, ClaveCentroCosto;";
    ...
    while ($rs = mysql_fetch_array($query_bitacoras)) {
        ...
        isset($rs['IdEnvio']) && $rs['IdEnvio'] != ""/* Si ya fue enviado el equipo */
    ...
    while ($rs = mysql_fetch_array($result)) {
        if ($rs['id_almacen'] != $IdAlmacenDestino) $almacenes .= "<option value='" . $rs['id_almacen'] . "'>" . $rs['nombre_almacen'] . "</option>";   
    }
...
<html lang="es">
    ...
    <body>
        <div class="principal">     
            <a href='reportes/SolicitudEquipo.php?noSolicitud=<?php echo $idSolicitud; ?>' target="_blank" title='Reporte' style="float: right;">
                <img src="resources/images/icono_impresora.png" width="30" height="30" />
            </a>
            <div class="p-4 bg-light rounded">
                ...
                <form id='formSerie' name='formSeries'>                
                            ...
                            <tbody>
                                ...
                                while ($rs = mysql_fetch_array($query)) {
                                    for ($j = 0; $j < intval($rs['cantidad_autorizada']); $j++, $contador++) {
                                        ...
                                        ?>
                                        <td align="center">
                                            <a href="#" title="Ver existencias" onclick="cambiarContenidos('ventas/verificarExistenciasTipoModelo.php?tipo=0&modelo=<?php echo $rs['parte']; ?>&idsol=<?php echo $idSolicitud; ?>','Ver existencias');return false;">
                                                <img src="resources/images/existencias.png" width="28" height="28"/>
                                            </a>
                                        </td>
                                        <?
                                        }
                        ...
                        <table id="tAlmacen" class="table">
                            <thead class="thead-dark">
                                <tr>
                                    ...
                                    echo "<th>Solicitados</th>";
                                    echo "<th>Surtir</th>";
                                    echo "<th>Almacén</th>";
                                    echo "<th>Existencias</th>";
                                    echo "<th>Desasociar</th>";
                                    if($permisos_grid->tienePermisoEspecial($_SESSION['idUsuario'], 35)){
                                        echo "<th> </th>";
                                    }
                                    ?>
                            ...
                            <tbody>
                                ...
                                while ($rs = mysql_fetch_array($query)) {
                                    ...
                                    if (isset($rs['IdAlmacen']) && $rs['IdAlmacen'] != "") {
                                        ...
                                        if (isset($rs['IdEnvio']) && $rs['IdEnvio'] != "" && $rs['cantidad_surtida'] == $rs['enviados']) {
                                            ...
                                            if ($checked == "" || $listo_enviar) {//Si no esta marcado como no surtir
                                                echo "1.<select id='almacen_sol2_$contador2' name='almacen_sol2_$contador2' style='width:150px;' 
                                                onchange='getExistenciasAlmacen(\"existencia2_$contador2\",\"almacen_sol2_$contador2\",\"modelo2_$contador2\");
                                                    ponerCantidad(\"cantidad2_$contador2\",\"cantidad2_sur_$contador2\",\"almacen_sol2_$contador2\");'>                                                
                                                $almacenes
                                                </select>";
                                    ...
                                    if ($faltan_asignar) {
                                        ...
                                        if($permisos_grid->tienePermisoEspecial($_SESSION['idUsuario'], 35)){
                                        ?>
                                        <td><a href="#" title="Ver existencias" onclick="lanzarPopUp('Ver existencias', 'ventas/verificarExistenciasTipoModelo.php?tipo=<?php echo $rs['IdTipoComponente']; ?>&modelo=<?php echo $rs['parte']; ?>');
                                    return false;"><img src="resources/images/existencias.png" width="28" height="28"/></a></td>
                                        <?
                                        }
                                        echo "</tr>";
                    ...
                    <input type='button' class='btn btn-default' id='cancelar_series' name='cancelar_series' value='Cancelar' onclick="cambiarContenidos('ventas/list_sol_equipo.php', 'Solicitudes');
                                return false;"/>                                
                    <?php
                    if ($permisos_grid->tienePermisoEspecial($_SESSION['idUsuario'], 1)) {//Si tiene permiso de asignar series
                        echo "<input type='submit' class='btn btn-default' id='submit_series' name='submit_series' onclick='cambiarTipoAccion(1);' value='Guardar'/>";
                    }
                    if (($series_capturadas || $hay_componentes) && $permisos_grid->tienePermisoEspecial($_SESSION['idUsuario'], 2)) {/* Si todas las series estan capturadas, entonces ya se pueden transferir los equipos */
                        ...
                        echo "<input type='submit' class='' id='mover_series' name='mover_series' onclick='cambiarTipoAccion(2);' value='Enviar a cliente' />";
                    }
                    ?>
                    </div>
                </form>
