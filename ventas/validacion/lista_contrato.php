...
<html lang="es">
    <head>
        ...
        <script type="text/javascript" language="javascript" src="<?php echo $back; ?>resources/js/paginas/validacion/lista_contrato.js"></script>        
    </head>
    <body>        
        <fieldset>
            <legend>Contrato</legend>
            <?php if($permisos_grid->tienePermisoEspecial($_SESSION['idUsuario'], 15)){ ?>
            <img class="imagenMouse" src="<?php echo $back; ?>resources/images/add.png" title="Nuevo cliente" onclick='cambiarContenidoValidaciones("contrato2", "<?php echo $back; ?>ventas/validacion/alta_contrato.php?Nuevo=true&idCliente=<?php echo $ClaveCliente; ?>", $("#idTicket").val(), null, true);
            ...
            if(mysql_num_rows($query) > 0){
                ...
                while ($rs = mysql_fetch_array($query)) {
                    ...
                    echo "<td align='center' scope='row'><input type='radio' id='check_contrato_".$rs['NoContrato']."' name='check_contrato' 
                        onclick='$(\"#equipos_p2\").empty(); $(\"#servicios_p2\").empty(); $(\"#servicios_g2\").empty(); cargarDependencia(\"anexo2\",\"".$back."ventas/validacion/lista_anexo.php?idCliente=".$ClaveCliente."\",\"".$rs['NoContrato']."\",\"check_contrato_".$rs['NoContrato']."\",\"".$_POST["idTicket"]."\"); '/></td>";
                    ...
                    echo "<a href='#' onclick='cambiarContenidoValidaciones(\"contrato2\",\"".$back."ventas/validacion/alta_contrato.php?idCliente=$ClaveCliente\", $(\"#idTicket\").val(),\"" . $rs['NoContrato'] . "\",false); return false;'>";
