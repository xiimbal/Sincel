...
$pagina_lista = "catalogos/lista_chofer.php";
        ...
        <div class="principal">
            ...
            if (isset($_POST['id'])) {
                ...
                if ($obj->getRegistroById($_POST['id'])) {
                    ...
                    $porcentaje = $obj->getPorcentajeDesc();
                    ...
                    $proveedorF = $obj->getProveedorFactura();
                }

                if ($domicilioTurno->getRegistroById($_POST['id'])) {
                    $turno = $domicilioTurno->getTurno();
                    $campania = $domicilioTurno->getCampania();
                    $area = $domicilioTurno->getArea();
                    $idVehiculo = $domicilioTurno->getVehiculo();
            ...
            <form id="formUsuario" name="formUsuario" action="/" method="POST">
                <table style="width: 95%;">
                    ...
                    <tr>
                        ...
                        <td><input type="text" id="rfc" name="rfc" value="<?php echo $rfc; ?>" minlength="10" maxlength="13"/></td>
                        <td><label for="puesto">Tipo de Chofer</label></td>
                        <td>
                            <select id="puesto" name="puesto" class="filtro">
                                ...
                                while ($rs = mysql_fetch_array($query)) {
                                    ...
                                    if ($rs['IdPuesto'] != 14 && ($rs['IdPuesto'] == 101 || $rs['IdPuesto'] == 108 || $rs['IdPuesto'] == 109)) {
                        ...
                        <td>Porcentaje de descuento por financial</td>
                        <td>
                            <input type="number" id="PorcentajeDesc" name="PorcentajeDesc" value="<?php echo $porcentaje; ?>" step="any"/>
                        </td>
                    </tr>
                    <tr>
                        <td><label for="vehiculos">Vehículos</label></td>
                        <td><select id="vehiculo" name="vehiculo" style="width: 180px;" class="filtro">
                                <option value="0">Seleccione un vehículo</option>
                                <?php
                                $catalogo1 = new Catalogo();
                                $query1 = $catalogo1->getListaAlta("c_vehiculo", "Modelo");
                                while ($rs = mysql_fetch_array($query1)) {
                                    $s = "";
                                    $conChofer = 0;
                                    $queryv = $catalogo1->obtenerLista("SELECT * FROM c_domicilio_usturno WHERE IdVehiculo > 0;");
                                    while ($rsv = mysql_fetch_array($queryv)) {
                                        if ($rsv['IdVehiculo'] == $rs['IdVehiculo']) {
                                            $conChofer = 1;
                                        }
                                    }
                                    if ($idVehiculo != "" && $idVehiculo == $rs['IdVehiculo']) {
                                        $s = "selected";
                                        $conChofer = 0;
                                    }
                                    if ($conChofer == 0) {
                                        echo "<option value=" . $rs['IdVehiculo'] . " " . $s . ">" . $rs['Placas'] . " - " . $rs['Modelo'] . "</option>";
                                    }
                                }
                                ?>
                            </select>
                        ...
                        <td>
                            <select id="proveedorF" name="proveedorF" class="filtro">
                                ...
                                while($rs = mysql_fetch_array($result)){
                                    $s = "";
                                    if(!empty($proveedorF) && $proveedorF == $rs['ClaveProveedor']){
                                        $s = "selected";
                                    }
                ...
                <input type="submit" class="boton" value="Cancelar" onclick="cambiarContenidos('<?php echo $pagina_lista; ?>');
                       ...
                       echo "<input type='hidden' id='NoAlta' name='NoAlta' value='3'/> ";
                       ?>
            </form>
        </div>
    </body>
