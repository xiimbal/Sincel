...
class ReporteFacturacion {
    ...
    public function getTabla($prefactura) {
        ...
        //Consulta vieja
        /*$consulta = "SELECT f.Folio, pp.Folio AS FolioPago,
            ...
            f.folioFiscal AS FolioFiscal, pp.PathXML, pp.PathPDF, pp.PathPDFPre
            FROM c_factura AS f  
         ...
         //Consulta nueva by Kike
        //$where .= " AND f.Idfactura = 86988";
        $consulta = "SELECT f.IdFactura,f.Folio AS Fol_1,
            (SELECT CASE WHEN !ISNULL(u.IdUsuario) THEN CONCAT(u.Nombre,' ',u.ApellidoPaterno,' ',u.ApellidoMaterno) ELSE '$vendedor' END) AS ejecutivo, 
            CONCAT(u2.Nombre,' ',u2.ApellidoPaterno,' ',u2.ApellidoMaterno) AS Ejecutivo_atencion,
            (CASE WHEN !ISNULL(c.DiasCredito) AND DATEDIFF(NOW(),f.PeriodoFacturacion) > c.DiasCredito THEN 'red' 
            WHEN ISNULL(c.DiasCredito) AND DATEDIFF(NOW(),f.PeriodoFacturacion) > $dias_credito THEN 'red' ELSE 'normal' END) AS color,
            DATE(f.FechaFacturacion) AS FechaFacturacion, 
            DATE(f.PeriodoFacturacion) AS PeriodoFacturacion, 
            f.FechaCancelacion, f.NombreReceptor, f.NombreEmisor, f.Serie, f.CFDI33, c.ClaveCliente, 
            SUBSTRING(f.RFCEmisor,1,3) AS RFCEmisor, 
            FORMAT((f.Total/1.16),2) AS subtotal, 
            FORMAT(((f.Total/1.16)*.16),2) AS importe, 
            FORMAT(f.Total,2) as Total, 
            f.Total AS TotalSinFormato, f.RFCReceptor, f.IdFactura AS IdFactura, f.PathPDF AS PDF, f.PathXML AS XML, tf.TipoFactura, 
            (SELECT GROUP_CONCAT(DISTINCT(fndc2.Folio) SEPARATOR ', ') FROM c_factura AS fndc2 WHERE fndc2.IdFacturaRelacion = f.IdFactura AND fndc2.TipoComprobante = 'egreso' ) AS PagadoNDC, 
            (SELECT CASE WHEN f.TipoComprobante = 'ingreso' THEN 'F' ELSE 'NDC' END) AS TipoComprobante, 
            (SELECT CASE WHEN f.EstadoFactura = 0 THEN 'C' WHEN f.PendienteCancelar = 1 THEN 'Pendiente Cancelar' WHEN f.TipoComprobante <> 'ingreso' THEN 'NDC' ELSE (SELECT CASE WHEN f.EstatusFactura = 3 THEN 'INC' ELSE (SELECT CASE WHEN f.FacturaPagada = 0 THEN 'NP' ELSE 'P' END) END) END) AS EstadoFactura, 
            (SELECT CASE WHEN f.FacturaEnviada = 1 THEN 'Si' ELSE 'No' END) AS Enviado, 
            ...
            LEFT JOIN c_usuario AS u2 ON u2.IdUsuario = c.EjecutivoAtencionCliente
            LEFT JOIN c_tipofacturaexp AS tf ON f.TipoArrendamiento = tf.IdTipoFactura 
            $where GROUP BY f.IdFactura ORDER BY f.Folio DESC $having";
    ...
    public function getTabla_kike($net,$idFac) {  
            $consulta = "SELECT f.IdFactura,
                        CASE WHEN (SELECT sum(ImportePagado) AS t_pagos FROM 
                            (SELECT ImportePagado FROM c_multipagosdetalle WHERE IdFactura = '$idFac' 
                                UNION ALL SELECT ImportePagado FROM c_pagosparciales WHERE IdFactura = '$idFac' ) t) IS NULL
                                THEN 0.00
                                ELSE (SELECT sum(ImportePagado) AS t_pagos FROM 
                            (SELECT ImportePagado FROM c_multipagosdetalle WHERE IdFactura = '$idFac' 
                                UNION ALL SELECT ImportePagado FROM c_pagosparciales WHERE IdFactura = '$idFac' ) t)
                                END
                            AS t_pagos,
                            ((SELECT Total FROM c_factura WHERE Idfactura = '$idFac')-
                            CASE WHEN (SELECT SUM(ImportePagado) FROM (SELECT ImportePagado FROM c_multipagosdetalle WHERE IdFactura = '$idFac' 
                                UNION ALL SELECT ImportePagado FROM c_pagosparciales WHERE IdFactura = '$idFac')t) IS NULL 
                                THEN 0.00 
                                Else (SELECT SUM(ImportePagado) FROM (SELECT ImportePagado FROM c_multipagosdetalle WHERE IdFactura = '$idFac' 
                                UNION ALL SELECT ImportePagado FROM c_pagosparciales WHERE IdFactura = '$idFac')t)
                                END
                            ) AS porPagar,
                                f.Total,
                                 CASE 
                                    WHEN (
                                    SELECT (GROUP_CONCAT(CONCAT('',CONCAT('$', FORMAT(mpd.ImportePagado, 2)),
                                                ' [',CAST(DATE(mpp.FechaPago) AS CHAR),'] (', mpp.Observaciones,
                                                ') (',mpp.Referencia,') / ',mpp.UsuarioCreacion) SEPARATOR ','))     
                                    FROM c_factura AS f1
                                    INNER JOIN c_multipagosdetalle AS mpd ON mpd.IdFactura = f1.IdFactura
                                    INNER JOIN c_multipagosparciales AS mpp ON mpd.IdMPagoParcial = mpp.IdMPagoParcial
                                    WHERE f1.IdFactura = '$idFac') IS NULL
                                    THEN '--'
                                    ELSE (SELECT (GROUP_CONCAT(CONCAT('',CONCAT('$', FORMAT(mpd.ImportePagado, 2)),
                                                ' [',CAST(DATE(mpp.FechaPago) AS CHAR),'] (', mpp.Observaciones,
                                                ') (',mpp.Referencia,') / ',mpp.UsuarioCreacion) SEPARATOR ','))     
                                    FROM c_factura AS f2
                                    INNER JOIN c_multipagosdetalle AS mpd ON mpd.IdFactura = f2.IdFactura
                                    INNER JOIN c_multipagosparciales AS mpp ON mpd.IdMPagoParcial = mpp.IdMPagoParcial
                                    WHERE f2.IdFactura = '$idFac') 
                                    END AS pagosMulti,
                                    CASE
                                    WHEN(
                                    SELECT (GROUP_CONCAT(CONCAT('',CONCAT('$', FORMAT(cp.ImportePagado, 2)),
                                                ' [',CAST(DATE(cp.FechaPago) AS CHAR),'] (', cp.Observaciones,
                                                ') (',cp.Referencia,') / ',cp.UsuarioCreacion) SEPARATOR ',')) 
                                    FROM c_factura AS f3
                                    INNER JOIN c_pagosparciales AS cp ON cp.IdFactura = f3.IdFactura
                                    WHERE f3.IdFactura = '$idFac') IS NULL
                                    THEN '--'
                                    ELSE (SELECT (GROUP_CONCAT(CONCAT('',CONCAT('$', FORMAT(cp.ImportePagado, 2)),
                                                ' [',CAST(DATE(cp.FechaPago) AS CHAR),'] (', cp.Observaciones,
                                                ') (',cp.Referencia,') / ',cp.UsuarioCreacion) SEPARATOR ',')) 
                                    FROM c_factura AS f4
                                    INNER JOIN c_pagosparciales AS cp ON cp.IdFactura = f4.IdFactura
                                    WHERE f4.IdFactura = '$idFac')
                                    END AS pagosIndi
                                 FROM c_factura AS f WHERE f.IdFactura = '$idFac'";
            $catalogo = new CatalogoFacturacion();
