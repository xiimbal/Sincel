...
class XMLReadAbraham {
    ...
    public function ReadXML($xml) {        
        ...
        if(!$xml != FALSE){
            ...
            $this->noAprobacion = $attrs['noAprobacion'];
            $this->anoAprobacion = $attrs['anoAprobacion'];
            $this->formaDePago = $attrs['formaDePago'];
            $this->subTotal = $attrs['subTotal'];
            $this->total = $attrs['total'];
            $this->tipoDeComprobante = $attrs['tipoDeComprobante'];
            $this->metodoDePago = $attrs['metodoDePago'];
            ...
            $attrs = $xml->Emisor->RegimenFiscal->attributes();
            $this->Regimen = $attrs['Regimen'];
            $attrs = $xml->Emisor->ExpedidoEn->attributes();
            ...
            foreach ($xml->Conceptos->children() as $a => $b) {
                $attrs = $b->attributes();
                $array = Array((float)$attrs['cantidad'], (float)$attrs['unidad'], $attrs['descripcion'], (float)$attrs['valorUnitario'], (float)$attrs['importe']);
    ...
    public function ReadXMLSinValidacion($xml) {
        try{
            $xml = simplexml_load_string($xml);
        }catch(Exception $e){
        ...
        foreach ($xml->Conceptos->children() as $a => $b) {
            $attrs = $b->attributes();
            $cantidad = str_replace(",", "", $attrs['cantidad']);
            
            $array = Array($cantidad, $attrs['unidad'], $attrs['descripcion'], (float)$attrs['valorUnitario'], (float)$attrs['importe']);
            //print_r($array);
            //echo "<br/>".$attrs['cantidad'].",,".$attrs['unidad'].",,".$attrs['descripcion'].",,".$attrs['valorUnitario'].",,".$attrs['importe'];
            array_push($this->conceptos, $array);
        }
        $attrs = $xml->Impuestos->Traslados->Traslado->attributes();
        return true;
