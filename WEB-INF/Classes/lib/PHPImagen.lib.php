...
class Imagen {
    ...
    private function getResources($file) {
        ...
        switch ($info['extension']) {
            case 'jpg': $r[0] = "imagecreatefromjpeg";
                $r[1] = "imageJpeg";
                $r[2] = "jpeg";
                break;
            case 'gif': $r[0] = "imagecreatefromgif";
                $r[1] = "imageGif";
                $r[2] = "gif";
                break;
            case 'png': $r[0] = "imagecreatefrompng";
                $r[1] = "imagePng";
                $r[2] = "png";
                break;
            default: $this->info['basename'] = $this->info['basename'] . ".jpg";
                $this->error("El tipo de archivo definido no es v�lido");
                return false;
                break;
        }
        $r[3] = $info;
    ...
    public function doSave($destination, $quality = null) {
        ...
        if (empty($info['filename']))
            $this->error("Para guardar se debe especificar un nombre de archivo v�lido");
    ...
    public function watermark($file, $posx = 0, $posy = 0, $loop = false, $rotation = 0) {
        ...
        if ($posx < 0)
            $posx = ($this->rw - $wm_w) + $posx;
        if ($posy < 0)
            $posy = ($this->rh - $wm_h) + $posy;
    ...
    public function textmark($text, $color, $size, $font = null, $rot = 0, $posx = 0, $posy = 0) {
        ...
        $this->error("El color de texto debe ser indicado en formato RGB o HTML v�lido");
        return false;
    }
    ...
    public function colorize($color, $exact = false) {
        ...
        $this->error("El nuevo color de imagen debe ser indicado en formato RGB o HTML v�lido");
        return false;
    }
