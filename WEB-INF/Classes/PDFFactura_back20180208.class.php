...
class PDFFactura extends FPDF {
    ...
    function CrearPDF33() { //Encabezado es correcto estamos en factura no?, no, ahorita la que viste estaba en pref copia las líneas no? es que se traba a veces conmigo
        ...
        for ($i = 0; $i < count($this->tabla); $i++) {
            ...
            if ($array[5] == "1") {//Si es un encabezado                
                ...
                if (strpos($array[0], ',') !== false) {
                    ...
                    $floatVal = floatval($array[0]);
                    if ($floatVal && intval($floatVal) != $floatVal) {
                        $this->Cell(20, $nb, utf8_decode(number_format((float) $array[0], 4)), 1, 0, 'R', false);
                    } else {
                        $this->Cell(20, $nb, utf8_decode(number_format((int) $array[0])), 1, 0, 'R', false);
                    }//dale
                }
                $this->Cell(25, $nb, utf8_decode(substr($array[2],0,26)), 1, 0, 'C', false);
                $y1 = $this->GetY();
                $x1 = $this->GetX();
                //tendremos que ocupar toda una pagina, en la primer hoja son menos lineas, no? perame estoy haciendo cosas mal                
                $this->MultiCell(105, 5, str_replace("&amp;","&",str_replace("&quot;", '"', utf8_decode($concepto))), 1, 'L', false);
				$y_aux_final = $this->GetY();
                $this->SetY($y1);
                $this->SetX($x1 + 105);
                //echo "<br/><b>Escribiendo en Y ".  $this->GetY()."</b> -- ".$this->filas[0];
                $this->Cell(25, $nb, utf8_decode("$ " . number_format($array[5], 2)), 1, 0, 'R', false);
                $this->Cell(25, $nb, utf8_decode("$ " . number_format($array[6], 2)), 1, 1, 'R', false);
                ...
                for ($j = $j_aux; $j < count($this->filas); $j++) {//recorremos hasta la ultima(se puede meter a una funcion)   
                    ...
                    $this->Cell(20, $nb, utf8_decode(''), 1, 0, 'C', false);
                    $this->Cell(25, $nb, utf8_decode(''), 1, 0, 'C', false); 
                    //$y1 = $this->GetY(); //
                    //$x1 = 40;
					$y1 = $this->GetY();
					$x1 = $this->GetX(); 
                    
                    $this->MultiCell(105, 5, utf8_decode($this->filas[$j]), 1, 'L', false); //estos conceptos ocuparan una página
                    $this->SetY($y1); //a ver que pasa con un concepto :P
                ...
                if(isset($array[8]) && !empty($array[8])){//Si hay descuento por partida                    
                    ...
                    $this->SetXY($x1, $y1);
                    //Importe
                    $this->Cell(25, $nb, utf8_decode("$ " . number_format($descuento_partida, 2)), 'BR', 'R', 'R'); 
        ...
        $this->MultiCell(120, 5, utf8_decode("(" . $this->num_Letra . ")"), 0, 'L', false);
        $this->Cell(40, 5, utf8_decode('Moneda: (MXN) Peso'), 0, 0, 'L', false);
        $size = 120;
