...
class ReporteLectura {
            ...
            if ($particularServicio[$key] == 0) {//Solo se procesan los costos de servicios globales                
                if (isset($costoExcedentesBN[$key]) && isset($excedenteBNServicio[$key])) {
                    ...
                    if ($MostrarEquipos && ($excedentes > 0 || $imprimir_cero)) {
                        ...
                        $this->crearConceptoFactura($excedentes, $um, " PAGINAS IMPRESAS NEGRO: $value INCLUYE(" . $incluidosBNServicio[$key] . ")", $costoExcedentesBN[$key], $costoExcedentesBN[$key] * $excedentes, $NumeroFacturas, $NumeroConcepto);
                        $NumeroConcepto++;
                    }
                }
                ...
                if (isset($costoProBN[$key])) {
                    ...
                    if ($MostrarEquipos && ($value > 0 || $imprimir_cero)) {
                        ...
                        $this->crearConceptoFactura($value, $um, "PAGINAS IMPRESAS NEGRO: $value", $costoProBN[$key], $costoProBN[$key] * $value, $NumeroFacturas, $NumeroConcepto);
                        $NumeroConcepto++;
                    }
                }
            }
            ...
            if ($particularServicio[$key] == 0) {//Solo se procesan los costos de servicios globales                
                if (isset($costoExcedentesColor[$key]) && isset($excedenteColorServicio[$key])) {
                    ...
                    if ($MostrarEquipos && ($excedentes > 0 || $imprimir_cero)) {
                        ...
                        $this->crearConceptoFactura($excedentes, $um, " PAGINAS IMPRESAS COLOR: $value INCLUYE(" . $incluidosColorServicio[$key] . ")", $costoExcedentesColor[$key], $costoExcedentesColor[$key] * $excedentes, $auxFactura, $auxConcepto);
                        $NumeroConcepto++;
                    }
                }
                if (isset($costoProColor[$key])) {
                    ...
                    if ($MostrarEquipos && ($value > 0 || $imprimir_cero)) {
                        ...
                        $this->crearConceptoFactura($value, $um, "PAGINAS IMPRESAS COLOR: $value", $costoProColor[$key], $costoProColor[$key] * $value, $auxFactura, $auxConcepto);
                        $NumeroConcepto++;
                    }
                }
            }
            ...
            foreach ($array as $columna => $value) {
                ...
                if (isset($rentaAgrupacion[$fila][$columna]) && !isset($rentas_globales_procesadas[$columna])) {
                    ...
                    if ($mostrarRenta && ($subtotal > 0 || $imprimir_cero)) {
                        ...
                        $this->crearConceptoFactura($rentas, $unidad_medida, $concepto, $rentaAgrupacion[$fila][$columna], $subtotal, $auxFactura, $auxConcepto);
                    }
                }
                ...
                if ($mostrarContadores && isset($costoExcedentesBNAgrupacion[$fila][$columna])) {
                    ...
                    if ($mostrarRenta && ($subtotal > 0 || $imprimir_cero)) {
                        ...
                        $this->crearConceptoFactura($excedenteBNAgrupacion[$fila][$columna], $unidad_medida, $concepto, $costoExcedentesBNAgrupacion[$fila][$columna], $subtotal, $NumeroFacturas, $NumeroConcepto);
                        $NumeroConcepto++;
                    }
                }
                ...
                if ($mostrarContadores && isset($costoExcedentesColorAgrupacion[$fila][$columna])) {
                    ...
                    if ($mostrarRenta && ($subtotal > 0 || $imprimir_cero)) {
                        ...
                        $this->crearConceptoFactura($excedenteColorAgrupacion[$fila][$columna], $unidad_medida, $concepto, $costoExcedentesColorAgrupacion[$fila][$columna], $subtotal, $auxFactura, $auxConcepto);
                    }
                }
                ...
                if ($mostrarContadores && isset($costoProcesadasBNAgrupacion[$fila][$columna])) {
                    ...
                    if ($mostrarRenta && ($subtotal > 0 || $imprimir_cero)) {
                        ...
                        $this->crearConceptoFactura($impresionesBNAgrupacion[$fila][$columna], $unidad_medida, $concepto, $costoProcesadasBNAgrupacion[$fila][$columna], $subtotal, $NumeroFacturas, $NumeroConcepto);
                        $NumeroConcepto++;
                    }
                }
                ...
                if ($mostrarContadores && isset($costoProcesadasColorAgrupacion[$fila][$columna])) {
                    ...
                    if ($mostrarRenta && ($subtotal > 0 || $imprimir_cero)) {
                        ...
                        $this->crearConceptoFactura($impresionesColorAgrupacion[$fila][$columna], $unidad_medida, $concepto, $costoProcesadasColorAgrupacion[$fila][$columna], $subtotal, $auxFactura, $auxConcepto);
                    }
                }
                ...
                if (!$mostrarRenta && ($totalServicio > 0 || $imprimir_cero)) {
                    ...
                    $this->crearConceptoFactura(1, "Servicio", $concepto, $totalServicio, $totalServicio, $NumeroFacturas, $NumeroConcepto);
                    $NumeroConcepto++;
                }
            }
        }
    ...
    public function imprimirDetalleServicio($idKServicio, $idServicio, $prefijo, $impresionesBN, $impresionesColor, $costoRentaServicio, $costoExcedentesBN, $costoExcedentesColor, $costoProcesadosBN, $costoProcesadosColor, $particularServicio, $iva, $excedenteBNServicio, $excedenteColorServicio, $equiposPorServicio, $NumeroFacturas, $NumeroConcepto, $unidad_servicio, $prefijos, $incluidosBNServicio, $incluidosColorServicio, $mostrarRenta, $encabezado_variable, $incluidosColor, $incluidosBN, $imprimir_cero, $dividir_lecturas, $MostrarSeries, $seriesServicio, $MostrarModelo, $agrupar_color, $NumeroConceptosColor) {
        ...
        if (isset($costoRentaServicio[$idKServicio])) {
            ...
            if ($mostrarRenta && ($subtotal > 0 || $imprimir_cero)) {
                ...
                $this->crearConceptoFactura($rentas, $unidad_medida, $concepto, $costoRentaServicio[$idKServicio], $subtotal, $auxFactura, $auxConcepto);
            }
        }
        ...
        if ($mostrarContadores && isset($costoExcedentesBN[$idKServicio]) /* && $costoExcedentesBN[$idKServicio]!=0 && $excedenteBNServicio[$idKServicio] > 0 */) {
            ...
            if ($mostrarRenta && ($subtotal > 0 || $imprimir_cero)) {
                ...
                $this->crearConceptoFactura($excedenteBNServicio[$idKServicio], $unidad_medida, $concepto, $costoExcedentesBN[$idKServicio], $subtotal, $NumeroFacturas, $NumeroConcepto);
                $NumeroConcepto++;
            }
        }
        ...
        if ($mostrarContadores && isset($costoExcedentesColor[$idKServicio]) /* && $costoExcedentesColor[$idKServicio]!=0 && $excedenteColorServicio[$idKServicio] > 0 */) {
            ...
            if ($mostrarRenta && ($subtotal > 0 || $imprimir_cero)) {
                ...
                $this->crearConceptoFactura($excedenteColorServicio[$idKServicio], $unidad_medida, $concepto, $costoExcedentesColor[$idKServicio], $subtotal, $auxFactura, $auxConcepto);
            }
        }
        ...
        if ($mostrarContadores && isset($costoProcesadosBN[$idKServicio]) /* && $costoProcesadosBN[$idKServicio]!=0 && $impresionesBN > 0 */) {
            ...
            if ($mostrarRenta && ($subtotal > 0 || $imprimir_cero)) {
                ...
                $this->crearConceptoFactura($impresionesBN, $unidad_medida, $concepto, $costoProcesadosBN[$idKServicio], $subtotal, $NumeroFacturas, $NumeroConcepto);
                $NumeroConcepto++;
            }
        }
        ...
        if ($mostrarContadores && isset($costoProcesadosColor[$idKServicio]) /* && $costoProcesadosColor[$idKServicio]!=0 && $impresionesColor > 0 */) {
            ...
            if ($mostrarRenta && ($subtotal > 0 || $imprimir_cero)) {
                ...
                $this->crearConceptoFactura($impresionesColor, $unidad_medida, $concepto, $costoProcesadosColor[$idKServicio], $subtotal, $auxFactura, $auxConcepto);
            }
        }
        ...
        foreach ($conceptos_adicionales as $value) {
            ...
            foreach ($array_seleccionado as $key => $value2) {
                if ($value2 == 0) {
                    ...
                    //nivel, desc, canti, pu, unidad,
                    $this->crearConceptoFactura($array_aux[2], $array_aux[4], $array_aux[1], $array_aux[3], $subtotal, $auxFactura, $auxConcepto);
                    $numeroColspan = "12";
                    if(!$mostrarContadores){
                        $numeroColspan = "4";
                    }
    ...
    public function crearConceptoRentaFactura($particular, $cantidad, $unidad_medida, $descripcion, $IdKServicio, $costoRentaServicio, $idServicioByKServicio, $numFactura, $numConcepto) {
        ...
        if ($cobrarRenta) {
            ...
            $this->crearConceptoFactura($cantidad, $unidad_medida, $descripcion, $precio_unitario, $importe, $numFactura, $numConcepto);
        }
    }

    public function crearConceptoFactura($cantidad, $unidad_medida, $descripcion, $precio_unitario, $importe, $numFactura, $numConcepto) {        
    ...
    public function generarConsultaConRetiros($anexo, $cc, $cen_costo, $cliente, $contrato, $zona, $agrupar_equipo, $agrupar_localidad, $agrupar_cc, $agrupar_servicio, $agrupar_zona, $agrupar_tipo_servicio, $tipoDomicilioFiscal, $anio, $mes, $MostrarLocalidad) {
        ...
        $select = "SELECT * FROM (SELECT 
            'TodoJunto' AS Junto,
            (SELECT (CASE WHEN meq2.clave_cliente_nuevo = '$cliente' THEN 1 ELSE 0 END)
        ...
        $order_by .= "IdKServiciogim DESC, IdKServiciogfa DESC, IdKServicioim DESC, IdKServiciofa DESC";
        ...
        $consulta .= "UNION
            ...
            0 AS VentaDirecta, b.id_bitacora,
            ctt.NoContrato,
            e.Modelo AS Modelo,
            c.RFC,
            kacc.IdAnexoClienteCC,
            ...
            WHERE meq.FacturarMovimiento = 1 AND meq.clave_cliente_anterior = '$cliente' AND (ISNULL(meq.clave_cliente_nuevo) OR meq.clave_cliente_nuevo <> '$cliente')
            AND meq.Fecha >= DATE_ADD(DATE_ADD(MAKEDATE(YEAR(DATE_SUB('$anio-$mes-01',INTERVAL 1 MONTH)), 1), INTERVAL (MONTH(DATE_SUB('$anio-$mes-01',INTERVAL 1 MONTH)))-1 MONTH), INTERVAL (DAY(kacc.Fecha))-1 DAY) 
            AND (ISNULL(cs.id_solicitud) OR cs.id_tiposolicitud <> 4)                        
            AND meq.Fecha <= NOW()
            $where $having) 
            AS t WHERE ISNULL(EstabaConElCliente) OR EstabaConElCliente = 1 GROUP BY NoSerie ORDER BY $order_by;";
        //echo $consulta;
        return $consulta;
    }
    ...
    public function imprimirEncabezadoServicio($encabezados, $incluidosBN, $incluidosColor, $costoRentaServicio, $costoExcedentesBN, $costoExcedentesColor, $costoProcesadosBN, $costoProcesadosColor, $idKServicio, $idServicio, $agrupar_color, $numFactura, $numConcepto) {
        ...
        echo "</tr>";
        $this->crearConceptoFactura("", "", $encabezado, "", "", $numFactura, $numConcepto);
