        ...
        if ($this->condicionesPago != "") {
            ...
            if(isset($this->IdPreFactura) && !empty($this->IdPreFactura)){  
                ...
                if($factura->getRegistrobyID() && $factura->getDiasCredito() != ""){
                    $this->condicionesPago = $factura->getDiasCredito()." días";
                }
            }
    ...
    function CrearPDF_DetalleSerie($prefactura) { //Encabezado 
        ...
        for ($i = 0; $i < count($this->tabla); $i++) {
            ...
            if ($array[5] == "1") {//Si es un encabezado                
                ...
                $x1_inicial = $x1;
                if (strpos($array[0], ',') !== false) {
                    $this->MultiCell(15, $nb, utf8_decode($array[0]), 'L', 'C', false);
                } else {
                    $floatVal = floatval($array[0]);                    
                    if ($floatVal && intval($floatVal) != $floatVal) {                                                
                        $this->MultiCell(15, $nb, utf8_decode(number_format((float) $array[0], 4)), 'L', 'C', false);
                    } else {                       
                        $this->MultiCell(15, $nb, utf8_decode(number_format((int) $array[0])), 'L', 'C', false);
                    }//dale
                }              
                ...
                $this->SetXY($x1,$y1);
                $this->MultiCell(15, $nb, utf8_decode($array[1]), '', 'C', false);                
                //Concepto
                $x1+=15;
                $this->SetXY($x1,$y1);
                //tendremos que ocupar toda una pagina, si en la primer hoja son menos lineas
                $this->MultiCell(120, 5, utf8_decode($concepto), '', 'L', false);
                $x1+=120;
                $this->SetXY($x1,$y1);
                
                //Precio Unitario
                $this->MultiCell(25, $nb, utf8_decode("$ " . number_format($array[3], 2)), '', 'R', false);
                $x1+=25;
                $this->SetXY($x1, $y1);
                //Importe
                $this->MultiCell(25, $nb, utf8_decode("$ " . number_format($array[4], 2)), 'R', 'R', false);                
                ...
                if(isset($this->IdPreFactura) && !empty($this->IdPreFactura)){                   
                    ...
                    $consulta = "SELECT b.NoSerie, e.Modelo, cc.Nombre AS Ubicacion,fd.ContadorBNAnterior,fd.ContadorBN,fd.Ubicacion AS Area,f.MostrarUbicacion,
                            (CASE WHEN !ISNULL(fd.ContadorBNAnterior) AND !ISNULL(fd.ContadorBN) THEN fd.ContadorBN - fd.ContadorBNAnterior ELSE 0 END) AS ProcesadasBN,
                            fd.ContadorColorAnterior,fd.ContadorColor, 
                            (CASE WHEN !ISNULL(fd.ContadorColorAnterior) AND !ISNULL(fd.ContadorColor) THEN fd.ContadorColor - fd.ContadorColorAnterior ELSE 0 END) AS ProcesadasColor
                    ...
                    while($rs = mysql_fetch_array($result)){
                        ...
                        $this->SetXY($x1+=$width, $y1);
                        $this->MultiCell($width, $nb_aux, utf8_decode(number_format($rs['ProcesadasBN'],0) ), '', 'C', false);
                        $this->SetXY($x1+=$width, $y1);
                        ...
                        $this->SetXY($x1+=$width, $y1);
                        $this->MultiCell($width, $nb_aux, utf8_decode(number_format($rs['ProcesadasColor'],0)), 'R', 'C', false);
    ...
    function NbLines($w, $txt) {
        ...
        while ($i < $nb) {//ya recorde que es el 5, son los pixeles o algo así verticalmente por eso lo multiplico por 5 
            if (($nl * 5) > (297 - $this->GetY()) && $nl_auxiliar == 0) {//297 es el limite vertical
                ...
                $return = $nl; //me oy a cambiar de conexion, pero sigo conectado
                $nl = 1; //sigue si 
                //$array_aux++;
