...
class MultiPagosParciales {
    ...
    private $ImporteGeneral; //importe total de las facturas que hay en el pago.

    //variables para agregar a la tabla detalle
    private $id_detalle;
    private $id_factura;
    private $impPagXFac;
    //private $impXPagXFac;
    ...
    public function getCanceladas($facturas){
        //***********************************************************************************************  *JT 18/10/18
        $consulta = "SELECT pp.FechaCreacion,
        (SELECT CASE WHEN pp.FechaCreacion<'2018-09-18 11:07:20' 
                    THEN CONCAT(cse.Prefijo,pp.Folio) 
                WHEN pp.FechaCreacion>'2018-09-18 11:07:20' 
                    THEN CONCAT(cs.Prefijo,pp.Folio) END) AS UNI,
        f.IdFactura,f.Folio,pp.IdMPagoParcial AS id_pago,DATE(pp.FechaPago) AS fechapago,pp.IdFormaPago,                
            (SELECT CASE WHEN f.EstadoFactura = 0 
                        THEN 'C' 
                    WHEN f.PendienteCancelar = 1 
                        THEN 'Pendiente Cancelar' 
                    WHEN f.TipoComprobante <> 'ingreso' 
                        THEN 'NDC' 
                    ELSE (SELECT CASE WHEN f.EstatusFactura = 3 
                            THEN 'INC' 
                        ELSE (SELECT CASE WHEN f.FacturaPagada = 0 
                            THEN 'NP' 
                        ELSE 'P' END) END) END) AS EstadoFactura,
        ...
        pp.FolioFiscal, pp.PathPDF, pp.PathXML,pp.PathPDFPre, CONCAT (cs.Prefijo, ' - ' , pp.Folio) AS FolioPP
        FROM c_factura AS f 
        INNER JOIN c_multipagoscanceladosdetalle as mpd ON mpd.IdFactura = f.IdFactura
        INNER JOIN c_multipagosparcialescancelados AS pp ON pp.IdMPagoParcial = mpd.IdMPagoParcial
        LEFT JOIN c_serie AS cse On cse.IdSerie = pp.IdSerie 
        LEFT JOIN c_seriepago AS cs On cs.IdSerie = pp.IdSerie 
        LEFT JOIN c_cliente as c ON c.RFC = (SELECT MIN(RFC) FROM c_cliente WHERE RFC = f.RFCReceptor AND Activo = 1) WHERE pp.IdsFacturas = '$facturas'
        GROUP BY mpd.IdMPagoParcial; ";
        //***********************************************************************************************************************
     ...
     public function getCanceladasMulti($factura){
        //***********************************************************************************************  *JT 18/10/18
        $consulta = "SELECT pp.FechaCreacion,
        (SELECT CASE WHEN pp.FechaCreacion<'2018-09-18 11:07:20' 
                    THEN CONCAT(cse.Prefijo,pp.Folio) 
        ...
        LEFT JOIN c_serie AS cse On cse.IdSerie = pp.IdSerie 
        LEFT JOIN c_seriepago AS cs On cs.IdSerie = pp.IdSerie 
        LEFT JOIN c_cliente as c ON c.RFC = (SELECT MIN(RFC) FROM c_cliente WHERE RFC = f.RFCReceptor AND Activo = 1) WHERE pp.IdsFacturas LIKE '%".$factura."%'
        GROUP BY mpd.IdMPagoParcial; ";
        //***********************************************************************************************************************
        $catalogo = new CatalogoFacturacion();
        // echo $consulta;
    ...
    public function CancelarPago(){
      ...
      //$factura = new Factura_NET();
            ...
            if (isset($this->ImporteFaltante)){
                $ImportePorPagar = (float) $this->ImporteFaltante;
            }
            if (!isset($this->ImporteGeneral) || $this->ImporteGeneral == "") {
                $this->ImporteGeneral = "null";
            }
        ...
        //    }
            ...
            $consulta = "INSERT INTO c_multipagosparcialescancelados(FolioFiscal,PathXML,PathPDF,FechaTimbrado,IdsFacturas,
                ...
                VALUES('".$this->FolioFiscal."','".$this->PathXML."','".$this->PathPDF."','".$this->FechaTimbrado."','".$this->idsFacturas."','" . $this->referencia . "','" . $this->observaciones . "','" . $this->importe . "',$ImportePorPagar," .
        ...
        if ($this->id_pago != NULL && $this->id_pago != 0) {
            echo "This id_pago ".$this->id_pago ;
            // $deleteDetalle = "DELETE FROM c_multipagosdetalle WHERE IdMPagoParcial = $pagoBorrar;";
            //$catalogo->obtenerLista($deleteDetalle);
            //$delete = "DELETE FROM c_multipagosparciales WHERE IdMPagoParcial = $pagoBorrar;";
    ...
    public function CancelarPago2($pago){
        $catalogo = new CatalogoFacturacion();
        if ($pago != NULL && $pago != 0) {
            $delete = "DELETE FROM c_multipagosparciales WHERE IdMPagoParcial = $pago;";
            $catalogo->obtenerLista($delete);
            return true;
        }
        return false;
    ...
    public function CancelarPagoFac($idpago){
            $factura = new Factura_NET3();
            $impXPagXFac = "null";
            if (isset($this->ImporteFaltante)) {
                $impXPagXFac = (float) $this->ImporteFaltante;
            }
            if (!isset($this->id_mpago)) {
                $consulta = "SELECT IdMPagoParcial AS IdMPago from c_multipagosparcialescancelados order by IdMPagoParcial desc limit 1";
                $catalogo = new CatalogoFacturacion();
                $query = $catalogo->obtenerLista($consulta);
                while ($rs = mysql_fetch_array($query)) {
                    $this->id_mpago = $rs['IdMPago'];
                    //echo "ID_MPAGO ". $rs['IdMPago'];
                    //return $query;
                }     
            }
            /*if (!isset($this->impPagXFac) || $this->impPagXFac == "") {
                $this->impPagXFac = "null";
            }*/
            ...
            $consulta = "INSERT INTO c_multipagoscanceladosdetalle(IdFactura, ImportePagado, ImportePorPagar, FechaPago, IdMPagoParcial) VALUES ('".$this->id_factura."','" . $this->impPagXFac . "','$impXPagXFac'," .$this->fechapago . ",'".$this->id_mpago."');";
            $catalogo = new CatalogoFacturacion();

        //echo "Error: ".$consulta;
        $this->id_pago = $catalogo->insertarRegistro($consulta);
        if ($this->id_pago != NULL && $this->id_pago != 0) {
            //echo "ID_PAGO ". $this->id_pago;
            $deleteDetalle = "DELETE FROM c_multipagosdetalle WHERE IdFactura = $this->id_factura AND IdMPagoParcial = $idpago";
            $catalogo->obtenerLista($deleteDetalle);
            $update = "UPDATE c_factura SET FacturaPagada = 0 WHERE IdFactura = $this->id_factura;";
            $catalogo->obtenerLista($update);
            return true;
    ...
    public function getTablaMulti($factura,$net) {
        if (!$net) {
            ...
            pp.FolioFiscal, pp.PathPDF, pp.PathXML, pp.PathPDFPre, CONCAT (cs.Prefijo, ' - ' , pp.Folio) AS FolioPP
            FROM c_factura AS f 
            INNER JOIN c_multipagosdetalle as mpd ON mpd.IdFactura = f.IdFactura
            INNER JOIN c_multipagosparciales AS pp ON pp.IdMPagoParcial = mpd.IdMPagoParcial
            LEFT JOIN c_serie AS cse On cse.IdSerie = pp.IdSerie 
            LEFT JOIN c_seriepago AS cs On cs.IdSerie = pp.IdSerie 
            LEFT JOIN c_cliente as c ON c.RFC = (SELECT MIN(RFC) FROM c_cliente WHERE RFC = f.RFCReceptor AND Activo = 1) WHERE pp.IdsFacturas LIKE '%".$factura."%'
            GROUP BY mpd.IdMPagoParcial; ";
    ...
    public function getTabla($facturas,$net) {
        if (!$net) {
            $consulta = "SELECT f.IdFactura,f.Folio,pp.id_pago,DATE(pp.fechapago) AS fechapago, pp.idCuentaBancaria,
                CONCAT('$', FORMAT(pp.importe, 2)) AS importe,pp.observaciones,pp.referencia,c.NombreRazonSocial,
                pp.FolioFiscal, pp.PathPDF, pp.PathXML,pp.UsuarioCreacion,pp.FechaCreacion
    ...
    public function getDatosbyFactura_Kike($net) {
        $idFac = $this->id_factura;
        ...
        if (!$net) {
            $consulta = "SELECT f.IdFactura,f.Folio,pp.id_pago,DATE(pp.fechapago) AS fechapago,pp.importe,pp.observaciones,
                ...
                for ($i=0; $i < number_format($tam); $i++) {
                    if ($i==0) {
                    	if (isset($this->id_pago)) {
                    		$consulta .= (" WHERE f.IdFactura='" . $arr_fac[$i] . "' AND pp.IdMPagoParcial = '".$this->id_pago."'");
                    	}else{
                    		$consulta .= (" WHERE f.IdFactura='" . $arr_fac[$i] . "'");
                    	}
                    }else {
                    	if (isset($this->id_pago)) {
                    		$consulta .= (" OR f.IdFactura='" . $arr_fac[$i] . "' AND pp.IdMPagoParcial = '".$this->id_pago."'");
                    	}else{
                    		$consulta .= (" OR f.IdFactura='" . $arr_fac[$i] . "'");
                    	}
                        
                    }
                }

            $consulta .= " GROUP BY f.IdFactura LIMIT 0,".($tam - 1).";";
            $catalogo = new Catalogo();
        } else {
            /*$consulta = "SELECT f.IdFactura,f.Folio,pp.IdPagoParcial,DATE(pp.FechaPago) AS fechapago,pp.ImportePagado AS importe, pp.IdFormaPago,
                pp.Observaciones,pp.Referencia,c.NombreRazonSocial, dfe.IdDatosFacturacionEmpresa, c.RFC, 
                ...
                LEFT JOIN c_datosfacturacionempresa AS dfe ON dfe.RFC = f.RFCEmisor
                LEFT JOIN c_cliente as c ON c.RFC = (SELECT MIN(RFC) FROM c_cliente WHERE RFC = f.RFCReceptor AND Activo = 1)";*/
             $consulta = "SELECT f.IdFactura,f.Folio,pp.IdMPagoParcial,DATE(pp.FechaPago) AS fechapago,mpd.ImportePagado AS importe, pp.IdFormaPago,
                pp.Observaciones,pp.Referencia,c.NombreRazonSocial, dfe.IdDatosFacturacionEmpresa, c.RFC, 
                f.Total AS Total,
                (f.Total - (SELECT SUM(pp2.ImportePagado) FROM c_multipagosdetalle pp2 WHERE pp2.IdFactura = f.IdFactura)) AS Pagado,
                f.FechaFacturacion
                FROM c_factura AS f
                LEFT JOIN c_multipagosdetalle AS mpd ON mpd.IdFactura=f.IdFactura
                LEFT JOIN c_multipagosparciales AS pp ON pp.IdMPagoParcial = mpd.IdMPagoParcial 
                LEFT JOIN c_datosfacturacionempresa AS dfe ON dfe.RFC = f.RFCEmisor
                LEFT JOIN c_cliente as c ON c.RFC = (SELECT MIN(RFC) FROM c_cliente WHERE RFC = f.RFCReceptor AND Activo = 1)";
                ...
                $consulta .= " GROUP BY f.IdFactura ORDER BY f.FechaFacturacion DESC, f.IdFactura LIMIT 0,".($tam - 1).";";
            $catalogo = new CatalogoFacturacion();
        }
    ...
    public function getRegistrobyPagoID() {
        ...
        $consulta = ("SELECT cp.IdFormaPago,CONCAT(cf.Nombre,' ',cf.Descripcion) AS FormaPago
                            FROM c_multipagosparciales as cp
                            INNER JOIN c_formapago as cf ON cp.IdFormaPago = cf.IdFormaPago
                            WHERE IdMPagoParcial='".$this->id_pago."'");
        $catalogo = new CatalogoFacturacion();
    ...
    /*public function getIdMultiPago(){
        $consulta = "SELECT IdMPagoParcial AS IdMPago from c_multipagosparciales order by IdMPagoParcial desc limit 1";
        $catalogo = new CatalogoFacturacion();
        $query = $catalogo->obtenerLista($consulta);
        while ($rs = mysql_fetch_array($query)) {
            $this->id_mpago = $rs['IdMPago'];
        ...
        //echo "ID_MPAGO fuera ". $this->id_mpago;
        //return $query;
    }*/
    public function getFacturasbyID($factura, $id_pago){
        $consulta = "SELECT IdFactura, ImportePagado, ImportePorpagar, FechaPago FROM c_multipagosdetalle WHERE idFactura = $factura AND IdMPagoParcial = $id_pago";
        $catalogo = new CatalogoFacturacion();
        $query = $catalogo->obtenerLista($consulta);
        while ($rs = mysql_fetch_array($query)) {
            $this->id_factura = $rs['IdFactura'];
            $this->impPagXFac = $rs['ImportePagado'];
            $this->ImporteFaltante = $rs['ImportePorpagar'];
    ...
    public function getRegistrobyID($net) {
        if (!$net) {
            ...
            $consulta = "SELECT FolioFiscal,FechaTimbrado,Referencia AS referencia,PathXML,PathPDF,
                Observaciones AS observaciones,ImportePagado AS importe,ImportePorPagar AS ImportePorPagar ,DATE(FechaPago) AS fechapago,IdFormaPago, IdsFacturas,
                IdSerie,Folio,idCuentaBancaria,IdTipoCadena,CertPago,CadPago,SelloPago,RFCBancoEmisorOrd,NomBancoEmisorOrd,CtaOrdenante,
                'Ninguna' AS Pantalla,NOW() AS FechaUltimaModificacion,'' AS UsuarioUltimaModificacion,NOW() AS FechaCreacion,UsuarioCreacion 
                FROM c_multipagosparciales WHERE IdMPagoParcial='$this->id_pago';";
            $catalogo = new CatalogoFacturacion();
            if(isset($this->empresa) && $this->empresa != ""){
            $catalogo->setEmpresa($this->empresa);
             }
        ...
        while ($rs = mysql_fetch_array($query)) {
            ...
            $this->ImporteFaltante = $rs['ImportePorPagar'];
            $this->fechapago = $rs['fechapago'];
            $this->idsFacturas = $rs['IdsFacturas'];
            ...
            //$this->ImporteGeneral = (double) $rs['importe'] + (Double) $rs['ImportePorPagar'];
            return true;
        }
        return false;
    }
    ...
    public function verificaPagoMayor($pago, $idFactura, $idPago) {
        ...
        $consulta = "SELECT 
            ...
            ELSE 0 END) extra     
            FROM c_factura AS f
            LEFT JOIN c_multipagosdetalle AS pp ON pp.IdFactura = f.IdFactura
    ...
    public function nuevoRegistroPago($net) {
        $cat = new Catalogo();
        if (!$net) {
            //echo "entramos el el if";
            $consulta = "INSERT INTO c_multipagosparciales(referencia,observaciones,importe,fechapago,UsuarioCreacion,FechaCreacion,UsuarioUltimaModificacion,FechaUltimaModificacion,Pantalla)
                VALUES('" . $this->referencia . "','" . $this->observaciones . "','" . $this->importe . "','" . $this->fechapago . "','" . $this->UsuarioCreacion . "',NOW(),'" . $this->UsuarioUltimaModificacion . "',NOW(),'" . $this->Pantalla . "');";
            $catalogo = new Catalogo();
        } else {
            //echo "Entramos al else";
            //$factura = new Factura_NET3();
            $ImportePorPagar = "null";
            if (isset($this->ImporteGeneral)){
                $ImportePorPagar = (float) $this->ImporteGeneral - (float) $this->importe;
            }
            if (!isset($this->ImporteGeneral) || $this->ImporteGeneral == "") {
                $this->ImporteGeneral = "null";
            }
            ...
            $consulta = "INSERT INTO c_multipagosparciales(Referencia,Observaciones,ImportePagado,ImportePorPagar,FechaPago,IdsFacturas,idCuentaBancaria,
                IdFormaPago,IdTipoCadena,CertPago,CadPago,SelloPago,RFCBancoEmisorOrd,NomBancoEmisorOrd,CtaOrdenante,IdSerie,Folio,UsuarioCreacion,FechaCreacion)
                VALUES('" . $this->referencia . "','" . $this->observaciones . "',
                '" . $this->importe . "','$ImportePorPagar'," .$this->fechapago . ",'$this->idsFacturas'," . $this->cuentaBancaria . 
    ...
    public function nuevoPagoFac($net){
        if (!$net) {
            //echo "Entraste al if";
        }else{
            //echo "Entras al else";
            ...
            $impXPagXFac = "null";
            if ($factura->getRegistroById($this->id_factura)) {
                $impXPagXFac = (float) $factura->getTotal() - (float) $this->impPagXFac;
            }
            if (!isset($this->id_mpago)) {
                $consulta = "SELECT IdMPagoParcial AS IdMPago from c_multipagosparciales order by IdMPagoParcial desc limit 1";
                $catalogo = new CatalogoFacturacion();
            ...
            $consulta = "INSERT INTO c_multipagosdetalle(IdFactura, ImportePagado, ImportePorPagar, FechaPago, IdMPagoParcial) VALUES ('".$this->id_factura."','" . $this->impPagXFac . "','$impXPagXFac'," .$this->fechapago . ",'".$this->id_mpago."');";
            $catalogo = new CatalogoFacturacion();
        }
    ...
    private function verificarPagado($net) {
        if (!$net) {
            ...
            $consulta = "SELECT 
                ...
                IF((SUM(mpd.ImportePagado)-f.Total)>=0,1,0) AS Pagado
                FROM c_factura AS f
                LEFT JOIN c_multipagosdetalle AS mpd ON mpd.IdFactura=f.IdFactura
    ...
    public function deleteRegistroPago($net){
        if (!$net) {
            $consulta = ("DELETE FROM c_pagosparciales WHERE id_pago='" . $this->id_pago . "'");
            $catalogo = new Catalogo();
        }else{
            $consulta = ("DELETE FROM c_multipagosparciales WHERE IdMPagoParcial = '" . $this->id_pago . "'");
            $catalogo = new CatalogoFacturacion();
    ...
    public function deleteRegistroFac($net) {
        if (!$net) {
            $consulta = ("DELETE FROM c_pagosparciales WHERE id_pago='" . $this->id_pago . "'");
            $catalogo = new Catalogo();
        } else {
            $consulta= ("DELETE FROM c_multipagosdetalle WHERE IdFactura = '".$this->id_factura."' AND IdMPagoParcial = '" . $this->id_pago . "'");
            $catalogo = new CatalogoFacturacion();         
        }
        $query = $catalogo->obtenerLista($consulta);
        if ($query == 1) {
    ...
    public function updateRegistro($net) {
        if (!$net) {
            ...
            /*$consulta = "UPDATE c_multipagosparciales SET IdFactura='" . $this->id_factura . "',Referencia='" . $this->referencia . "',
            ...
            $consulta = "UPDATE c_multipagosparciales SET Referencia='" . $this->referencia . "',
                Observaciones='" . $this->observaciones . "',ImportePorPagar=$ImportePorPagar,IdFormaPago = $this->IdFormaPago, IdSerie=$this->IdSerie,
                IdTipoCadena = $this->IdTipoCadena, CertPago = '$this->CertPago', CadPago= '$this->CadPago', SelloPago = '$this->SelloPago',
                RFCBancoEmisorOrd = '$this->RFCBancoEmisorOrd', NomBancoEmisorOrd = '$this->NomBancoEmisorOrd', CtaOrdenante = '$this->CtaOrdenante',
                ImportePagado='" . $this->importe . "',FechaPago='" . $this->fechapago . "',idCuentaBancaria = $this->cuentaBancaria WHERE IdMPagoParcial='" . $this->id_pago . "'";
            $catalogo = new CatalogoFacturacion();
        }
    ...
    public function updateRegistroFac($net){
        if (!$net) {
            $consulta = ("UPDATE c_pagosparciales SET id_factura='" . $this->id_factura . "',referencia='" . $this->referencia . "',observaciones='" . $this->observaciones . "',
                importe='" . $this->importe . "',fechapago='" . $this->fechapago . "',
                UsuarioUltimaModificacion='" . $this->UsuarioUltimaModificacion . "',FechaUltimaModificacion=NOW(),Pantalla='" . $this->Pantalla . "' 
            ...
            $idPagDet = "";
            if (empty($idPagDet)) {
                $consulta = "SELECT idDetallePago AS idDetallePago FROM c_multipagosdetalle WHERE IdFactura = '".$this->id_factura."' AND IdMPagoParcial = '".$this->id_pago."'";
                $catalogo = new CatalogoFacturacion();
                $query = $catalogo->obtenerLista($consulta);
                while ($rs = mysql_fetch_array($query)) {
                    $idPagDet = $rs['idDetallePago'];
                }     

            }
            $consulta = "UPDATE c_multipagosdetalle SET IdFactura='" . $this->id_factura . "',ImportePagado='" . $this->impPagXFac . "',ImportePorPagar=$impXPagXFac, FechaPago='" . $this->fechapago . "' WHERE IdDetallePago = '".$idPagDet."' AND IdMPagoParcial='" . $this->id_pago . "'";
            $catalogo = new CatalogoFacturacion();
        }
    ...
    public function getNumeroParcialidad() {
        ...
        $consulta = "SELECT COUNT(*) AS Numero, SUM(mpd.ImportePagado) AS Pagado
                FROM c_multipagosdetalle as mpd INNER JOIN c_multipagosparciales as mpp
                ON mpd.IdMPagoParcial = mpp.IdMPagoParcial
                WHERE !ISNULL(mpp.FolioFiscal) AND mpd.IdFactura = $this->id_factura";
        $result = $catalogo->obtenerLista($consulta);
    ...
    //*************************************************** Metodo que obtiene el saldo anterior de los pagos para PrePagos  *JT 03/10/18
     public function getNumeroParcialidadPago() {
        ...
        $consulta = "SELECT COUNT(*) AS Numero, SUM(mpd.ImportePagado) AS Pagado
                FROM c_multipagosdetalle as mpd INNER JOIN c_multipagosparciales as mpp
                ON mpd.IdMPagoParcial = mpp.IdMPagoParcial
                WHERE mpd.IdFactura =$this->id_factura";
        //echo "<br>".$consulta;
        $result = $catalogo->obtenerLista($consulta);
        if ($rs = mysql_fetch_array($result)) {
...
//**************************************************************Metodo que obtiene datos del pago parcial cuando se edita un pago existente *JT 04/10/18
    ...
    $catalogo = new CatalogoFacturacion();
        ...
        $consulta = "SELECT COUNT(*) AS Numero,SUM(mpd.ImportePagado) AS SaldoAnterior
                          FROM c_multipagosdetalle as mpd INNER JOIN c_multipagosparciales as mpp
                          ON mpd.IdMPagoParcial = mpp.IdMPagoParcial
                          WHERE mpd.IdFactura = $this->id_factura AND mpp.IdMPagoParcial<  $this->id_pago";
        $result = $catalogo->obtenerLista($consulta);
        if ($rs = mysql_fetch_array($result)) {
...
//*************************************************************************** Metodo para obtener pagos anteriores      *JT 10/10/18
    public function getRegistrosPPAnteriores(){
        $Observaciones2="Pago realizado a partir de la nota de cr?ito con folio";
        $b=0;
        $catalogo = new CatalogoFacturacion();
        if(isset($this->empresa)){
            $catalogo->setEmpresa($this->empresa);
...
//***************************************************************************
    ...
    //importe general del pago de varias facturas
    ...
    //funciones get y set de las variables para guardar en detalle

    public function getId_detalle(){
        return $this->id_detalle;
    }
  
    public function getImportePagadoXFactura(){
        return $this->impPagXFac;
    }
    public function getImportePorPagarXFactura(){
        return $this->impXPagXFac;
    }
    public function getIdmpago(){
        return $this->id_mpago;
    }

    public function setId_detalle($id_detalle){
        $this->id_detalle = $id_detalle;
    }
    public function setImportePagadoXFactura($impPagXFac){
        $this->impPagXFac = $impPagXFac;
    }
    public function setImportePorPagarXFactura($impXPagXFac){
        $this->impXPagXFac = $impXPagXFac;
    }
    public function setIdmpago($id_mpago){
        $this->id_mpago = $id_mpago;
    }
    ...
    function getIdsFacturas(){
        return $this->idsFacturas;
    }
    function set_IdsFacturas($idsFacturas) {
