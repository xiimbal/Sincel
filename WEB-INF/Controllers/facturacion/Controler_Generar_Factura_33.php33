...
if (isset($_GET['id']) && $_GET['id'] != "") {
    ...
    $cadena_original = "||" . $xml->getVersion() . $anadirSerie . "|" . $xml->getFolio() ."|" . $xml->getFecha() . "|" . $xml->getFormaDePago() . "|" . $xml->getNoCertificado();
    $cadena_original.="|" . $xml->getSubTotal() . $anadirDescuento . "|" . $xml->getMoneda() . "|XYZ&&&|" . $xml->getTipoDeComprobante() . "|" . $xml->getMetodoDePago() . "|" . $xml->getLugarExpedicion();
    ...
    for ($i = 0; $i < count($array); $i++) {
        ...
        $cadena_original.=trim(preg_replace('/\s\s+/', ' ', "|" . $arr[1] . "|" . $arr[0] . "|" . $arr[2] . "|" . $arr[3] . "|" . $arr[4] . "|" . $arr[5] . "|" . $arr[6].$descuento_partida));
        $cadena_original.=trim(preg_replace('/\s\s+/', ' ', "|" . $total_partida . "|002|Tasa|0.160000|" . number_format($total_partida * 0.16,2,".","") ));
    ...
    $total = number_format( $subtotal - $descuento + $totalImpuestosTrasladadosSuma , 2, ".", "");    
