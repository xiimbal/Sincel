...
include_once("../../Classes/FacturaTimbradaUso.class.php");

//************************************************* *JT 25/10/18
$bandera = new FacturaTimbradaUso();

$posicion = $bandera->nuevoRegistroBandera(); 
if($posicion>1){
    $timpoEspera=$posicion*2;
    sleep($timpoEspera);
}
...
if (isset($_GET['id']) && $_GET['id'] != "") {
    ...
    if (isset($_POST['fac_sust']) && $_POST['fac_sust'] == "1") {
        $fac_sust = true;
        //echo "VALOR RECIBIDO FAC ".$_POST['fac_sust'] . " facturaasdasf " .$_POST['IdFacturaNET'];
    ...
    if ($factura->verificarFacturasDobles()) {
        ...
        if(!$bandera->liberarProcesoTimbreado())
            echo"<br>No se borro correctamente los registros";  
        return;
    }
    ...
    if (isset($IdSeriePrefijo) && $IdSeriePrefijo != "") {
        $cata = new Catalogo();
        $cata_f = new CatalogoFacturacion();
    ...
    if ($fac_sust && isset($_POST['IdFacturaNET']) && $_POST['IdFacturaNET'] != "0") {
        $factura_aux = new Factura_NET();
        if ($factura_aux->getRegistroById($_POST['IdFacturaNET'])) {
            $xml->setTipoRelacion($factura->getClaveTipoRelacion($factura->getTipoRelacion(), FALSE));
            $pdf->setTipoRelacion($factura->getClaveTipoRelacion($factura->getTipoRelacion(), true));
    ...
    if ($reporte->GenerarFactura($xmlcreado)) {
        ...
        if ($soap_timbrado == false) {
            ...
            if (isset($soap_timbrado['return']['timbre']) && $soap_timbrado['return']['timbre'] != "") {
                ...
                } else if ($fac_sust && !$ndc){
                    $pdf->setTitulo("FACTURA SUSTITUCIÓN");
                    $pdf->setNdc(false);
                    $pdf->setTipoComprobante("I Ingreso");
                }else{
                ...
                $tempDir = "../../../PDF/";
                $cadena = "https://verificacfdi.facturaelectronica.sat.gob.mx/default.aspx?re=" . $pdf->getRFC_Emisor() . "&rr=" . $pdf->getRFC_Receptor();
                $tot = $pdf->getTotal();
                $toa = explode(".", $tot);
                ...
                //FERNANDO
			    $sinletras = eregi_replace("[a-zA-Z]","",$factura->getFolio());
			    $contratofac = "UPDATE c_factura SET NoContrato = ".$ccf." WHERE Folio = ".$sinletras.";";
			    $cata_f->insertarRegistro($contratofac); 
			    //echo $contratofac;
...
//Aqui regresa la bandera a disponible
if(!$bandera->liberarProcesoTimbreado())
     echo"<br>No se borro correctamente los registros";   
