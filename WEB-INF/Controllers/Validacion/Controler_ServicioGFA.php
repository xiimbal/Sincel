...
include_once("../../Classes/ServicioGFA.class.php");
...
if (isset($_POST['eliminar']) && isset($_POST['IdKServicio'])) {
    ...
    if (mysql_num_rows($result) > 0) {
        ...
        if ($obj->getRegistroById($_POST['IdKServicio'])) {
            ...
            $result = $obj_aux->getSolicitudesAbiertasAsignadas($_POST['IdKServicio'], $obj->getIdServicioGFA());
            if (mysql_num_rows($result) > 0) {
    ...
    if (isset($parametros['tipo_servicioIM'])) {
        $obj->setIdServicioGFA($parametros['tipo_servicioIM']);
    } else {
        $obj->setIdServicioGFA($_POST['servicio']);
    }
