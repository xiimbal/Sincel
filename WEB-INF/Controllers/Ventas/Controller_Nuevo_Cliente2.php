    ...
    if (isset($parametros['id']) && $parametros['id'] != "") {//actualizar
        ...
        if($cliente_aux->getRegistroByRFC($cliente->getRFCD()) && $cliente_aux->getClaveCliente() != $parametros['id']){
            ...
            if($cliente->getModalidad() == "3"){                
                $cliente->setClaveCliente($cliente->getIdCliente());
                $cliente->setFormaPago($parametros['forma_pago']);
                $cliente->setMetodoPago($parametros['metodo_pago']);
                $cliente->setNumero_cuenta($parametros['numero_cuenta']);
                $cliente->setIdBanco($parametros['banco']);
                $cliente->setIdUsoCFDI($parametros['usoCFDI']);
                $cliente->setDias_credito($parametros['dias_credito']);
                $cliente->setIdFormaComprobante($parametros['forma_pago_complemento']);                
                $cliente->setIdCuentaBancariaCFDI33($parametros['cuenta_bancaria']);
                
                $cliente->newDatosFiscalesVentas();
                $cliente_aux = new ccliente();
                $cliente_aux->setClaveCliente($cliente->getClaveCliente());
                if($cliente_aux->getDatosFiscalesVenta()){
                    $cliente->editDatosFiscalesVentas();
                }else{
                    $cliente->newDatosFiscalesVentas();
                }
            }else{
                $cliente->deleteDatosFiscalesVentas();
            }         
        }
