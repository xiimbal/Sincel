...
if (isset($_GET['like'])) {
    ...
    if ($usuario->isUsuarioPuesto($id_usuario, 21)) {
        $query = $catalogo->obtenerLista("SELECT
            ...
            WHERE (t2.NoSerieEquipo = cinv.NoSerie OR p.ClaveEspEquipo = cinv.NoSerie) AND t2.EstadoDeTicket NOT IN(2,4) AND (ISNULL(nt.IdEstatusAtencion) OR nt.IdEstatusAtencion NOT IN(16,59)))
    ...
    if (isset($_GET['tipo']) && $_GET['tipo'] != 2) {
        $query2 = $catalogo->obtenerLista("SELECT 
            ...
            cs.id_solicitud,
            IF(ISNULL(csrg.IdSolicitudRetiroGeneral),'0',IF(csr.PendienteRetiro=0 AND csrg.Contestado=0,'1',IF(csr.PendienteRetiro=1 AND csrg.Contestado=1 AND csrg.Aceptada=1,'1',IF(csrg.Aceptada=1 AND csrg.Contestado=1 AND csr.IdAlmacen!=9 AND c_almacen.id_almacen=9,'1','0')))) AS MoverRojo,
            c_equipo.Modelo AS Modelo
            FROM c_almacen
            ...
            WHERE k_almacenequipo.NoSerie LIKE '%" . $_GET['like'] . "%' AND bit.Activo = 1 ORDER BY NombreAlmacen,NoSerie");
...
while ($rs = mysql_fetch_array($query2)) {
    if ($anterior_centro == $rs['NombreAlmacen']) {  
        ...
        if ($rs['MoverRojo'] == "1") {
            $pendiente = "<span style='color:red'>(Equipo en retiro)</span>";
            $no = "no";
        }
        ...
        if ($anterior_centro == $cc) {            
            if ($rs['NoSerie'] != null) {
                ...
                if(isset($rs['IdTicket']) && $rs['IdTicket'] != ""){
                    $pendiente = "<span style='color:red'> (En ticket abierto: ".$rs['IdTicket'].")</span>";
                    $no = "no";
                }
                if($rs['Demo'] == "1"){
                    $pendiente = "<span style='color:red'> (En demo)</span>";
            ...
            $json.="]},{\"id\":\"no" . $rs['ID'] . "\",\"text\":\"" . $cc . "&nbsp;<span style='color:blue'>(" . $rss['SUMA'] . ")</span>\",\"iconCls\":\"icon-edificio\",\"children\":[";            
            $id_centro = $id;
            $anterior_centro = $cc;
            $id++;
            $pendiente = "";
