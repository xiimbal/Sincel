...
if (isset($_POST['form'])) {
    ...
    if ($toners_solicitados <= $maximos_toner) {//El numero de toner solicitados es menor o igual al permitido
        ...
        if (isset($_POST['solicitud']) && $_POST['solicitud'] != "") {
            $consulta = "UPDATE c_solicitud SET comentario='" . $parametros['comentario_normal'] . "',comentario_creo='$comentarios',estatus=2,ClaveCliente='" . $cliente . "',
                id_almacen=$idAlmacen,id_tiposolicitud=" . $parametros['tipo_solicitud'] . ",fecha_regreso=$fecha_devolucion,IdFormaPago=" . $formasPago . ",
            ...
            if ($query == 1) {
                echo "Se guardó la solicitud " . $_POST['solicitud'] . " exitosamente.";
            } else {
                echo "<br/>Error: no se pudo guardar la solicitud " . $_POST['solicitud'] . " correctamente";
            }
            ...
            $consulta = "INSERT INTO c_solicitud(id_crea,comentario,comentario_creo,estatus,ClaveCliente,id_almacen,fecha_solicitud,id_tiposolicitud,
                fecha_regreso,IdFormaPago,dias_credito,dias_revision,Activo,UsuarioCreacion,FechaCreacion,UsuarioUltimaModificacion,FechaUltimaModificacion,Pantalla)
                VALUES('" . $_SESSION['idUsuario'] . "','" . $parametros['comentario_normal'] . "','$comentarios',0,'" . $cliente . "',$idAlmacen,NOW()," . $parametros['tipo_solicitud'] . ",$fecha_devolucion,"
                    . $formasPago . "," . $dias_credito . "," . $dias_revision . ",1,'" . $_SESSION['user'] . "',NOW(),'" . $_SESSION['user'] . "',NOW(),'PHP Nueva_Solicitud.php');";
            $id_solicitud = $catalogo->insertarRegistro($consulta);
            //echo $consulta;
            if ($id_solicitud == null || $id_solicitud == 0) {
                echo "<br/>Error: no se pudo insertar en la base de datos la solicitud correctamente";
                return null;
            } else {
                echo $id_solicitud;
            }
        ...
        if (isset($_POST['solicitud']) && $_POST['solicitud'] != "") {
            $result = $catalogo->obtenerLista("SELECT * FROM k_solicitud WHERE tipo=0 AND id_solicitud=" . $_POST['solicitud']);
