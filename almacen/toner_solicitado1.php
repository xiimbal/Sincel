        ...
        <div class="principal">            
            ...
            <table id="tAlmacen2" class="tabla_grid" style="display: none; width: 100%;">
                ...
                <tbody>
                    <?php
                    if ($idTicket == "") {
                        /*$consulta = "SELECT t.IdTicket,nt.IdNotaTicket,c.Modelo,cl.NombreRazonSocial,nr.CantidadSurtida,nr.Cantidad,(SELECT ar.Descripcion FROM c_area ar WHERE ar.IdArea=t.AreaAtencion)AS area,DATE(t.FechaHora) AS FechaHora
                                        ...
                                        t.NombreCentroCosto,t.DescripcionReporte,c.NoParte,nt.MostrarCliente,nt.Activo,nt.UsuarioSolicitud,nr.IdAlmacen,
                                        cl.ClaveCliente,kac.Ubicacion
                        ...
                        /*$consulta = "SELECT t.IdTicket,nt.IdNotaTicket,c.Modelo,cl.NombreRazonSocial,nr.CantidadSurtida,nr.Cantidad,(SELECT ar.Descripcion FROM c_area ar WHERE ar.IdArea=t.AreaAtencion)AS area,DATE(t.FechaHora) AS FechaHora
                                            ,CONCAT ('(',nr.Cantidad,') ',c.Modelo) AS refaccion,DATEDIFF(NOW(), t.FechaHora) AS diferencia,nt.IdEstatusAtencion
                                            ,t.EstadoDeTicket,cl.IdTipoCliente,cl.IdEstatusCobranza,t.TipoReporte,e.Nombre AS ultimoEstatus,
                                            nt.DiagnosticoSol,t.NombreCentroCosto,t.DescripcionReporte,c.NoParte,nt.MostrarCliente,nt.Activo,
                                            nt.UsuarioSolicitud,nr.IdAlmacen,cl.ClaveCliente,kac.Ubicacion
                                            FROM k_nota_refaccion nr,c_notaticket nt,c_ticket t,c_componente c,c_cliente cl,c_estado e, k_almacencomponente kac
                                            WHERE t.IdTicket=nt.IdTicket AND nt.IdNotaTicket=nr.IdNotaTicket AND nr.NoParteComponente=c.NoParte AND t.ClaveCliente=cl.ClaveCliente
                                            AND $cliente nt.IdEstatusAtencion=e.IdEstado AND (kac.id_almacen = nr.IdAlmacen AND kac.NoParte = c.NoParte)
