...
echo "<select id='tipo_servicio' name='tipo_servicio' onchange='cambiarContenidoServicio(\"div_servicioim\",\"cliente/validacion/alta_servicio_im.php?Nuevo=true&CC=$CC&Anexo=$ClaveAnexo\",
                                true,\"$ClaveAnexo\",this.value,\"$CC\");'>";
if ($nuevo == "si") {
...
if ($nuevo == "si" || $s != "") {    
    $result = $catalogo->getListaAlta("c_servicioim", "IdServicioIM");
...
if ($nuevo == "si" || $s != "") {
    $result = $catalogo->getListaAlta("c_serviciogim", "IdServicioGIM");
    if(mysql_num_rows($result) > 0){
...
if ($nuevo == "si" || $s != "") {
    $result = $catalogo->getListaAlta("c_serviciofa", "IdServicioFA");
    if(mysql_num_rows($result) > 0){
...
if ($nuevo == "si" || $s != "") {
    $result = $catalogo->getListaAlta("c_serviciogfa", "IdServicioGFA");
    if(mysql_num_rows($result) > 0){
...
if ($nuevo == "si" || $s != "") {
    $result = $catalogo->obtenerLista("SELECT * FROM c_serviciosve WHERE IdAnexoClienteCC = '$ClaveAnexo'");
    if(mysql_num_rows($result) < 1){
        echo "<option value='ve' $s>Servicios Especiales</option>";
    }
}
$s = "";
echo "</select>";
...
<html lang="es">
    <head>
        ...
        <script type="text/javascript" language="javascript" src="../resources/js/paginas/validacion/alta_servicioim.js"></script>
        <style>
            .chico{width: 80px;}
        </style>
    </head>
