...
while ($rs = mysql_fetch_array($query)) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)            
            ...
            ->setCellValue('N' . ($fila_inicial), $tipoComprobante)->setCellValue('O' . ($fila_inicial), $rs['ejecutivo']);
    $fila_inicial++;
}

cellColor($objPHPExcel, 'A1:F1', 'C0C0C0'); //TITULO REPORTE
cellColor($objPHPExcel, 'A'.$fila_inicial_backup.':O'.$fila_inicial_backup, 'C0C0C0'); //TITULO REPORTE
...
$objPHPExcel->getActiveSheet()->getStyle('A'.$fila_inicial_backup.':O'.$fila_inicial_backup)->applyFromArray($styleArray);/*Cabeceras de la tabla*/
$styleArray = getStyle(true, "000000", 9, "Arial", false);
$objPHPExcel->getActiveSheet()->getStyle('J1:O1')->applyFromArray($styleArray);/*Fecha y hora*/
