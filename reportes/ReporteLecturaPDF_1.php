...
$permisos_grid = new PermisosSubMenu(); $permisos_grid->setEmpresa($empresa);
$catalogo = new Catalogo(); $catalogo->setEmpresa($empresa);
$obj_cliente = new Cliente(); $obj_cliente->setEmpresa($empresa);
$obj_direccion = new Localidad(); $obj_direccion->setEmpresa($empresa);
$obj_contacto = new Contacto(); $obj_contacto->setEmpresa($empresa);
$obj_zona = new Zona(); $obj_zona->setEmpresa($empresa);
$reporte = new ReporteLectura(); $reporte->setEmpresa($empresa);
$caracteristicas = new EquipoCaracteristicasFormatoServicio();  $caracteristicas->setEmpresa($empresa);
$parametros = new Parametros(); $parametros->setEmpresa($empresa);
            ...
            <form id="<?php echo $form; ?>" name="<?php echo $form; ?>" action="<?php echo $action; ?>" method="POST" target="_blank">                                
                ...
                foreach ($tiposDomicilios as $value) {
                    ...
                    for($j=0; $j<$limit; $j++){                        
                        ...
                        $consulta = $reporte->generarConsultaConRetiros($anexo, $cc_aux, $cen_costo, $cliente, $contrato, $zona,
                                $agrupar_equipo, $agrupar_localidad, $agrupar_cc, $agrupar_servicio, $agrupar_zona, $agrupar_tipo_servicio, $value, $year, $month);
                        ...
                        if(mysql_num_rows ($result) > 0){
                            ...
                            $NumeroConcepto = 0; $descripcion_renta = ""; $idBitacoras = ""; $idBitacorasColor = ""; $idBitacorasRenta = "";
                ...
                <?php
                    while($rs = mysql_fetch_array($result)){
                            ...
                            if(!$nueva_hoja){/*Cerramos la ultima hoja abierta*/
                                ...
                                if(!$mostrarDetalleServicio && !$mostrarDetalleLocalidad && !$mostrarDetalleCC && !$mostrarDetalleZona){
                                    if(!$particularAnterior){                                        
                                        ...
                                        $reporte->setFields($fields);
                                        $reporte->crearConceptoRentaFactura($particularAnterior, $equiposPorServicio, $um, $descripcion_renta, 
                                                $IdKServicioAnterior, $costoRentaServicio, $idServicioByKServicio, $auxFactura, $auxConcepto);
                                        $fields = $reporte->getFields();
                                    }
                                }
                                $descripcion_renta = "";                                
                                ...
                                $incluidosBNServicio = array(); $incluidosColorServicio = array(); $seriesServicio = array();
                                $impresionesBNPorServicio = 0; $impresionesColorPorServicio = 0; $equiposPorServicio=0;
                                $idBitacoras = ""; $idBitacorasColor = ""; $idBitacorasRenta = "";
                                ?>
                        ...
                        if( (isset($contadorBN) && $contadorBN > 0) ){
                            if(isset($contadorColor) && $contadorColor > 0){
                                $idBitacoras .= ($rs['id_bitacora']."&&$contadorBN&&$contadorBNAnterior&&$contadorColor&&$contadorColorAnterior,");
                            }else{
                                $idBitacoras .= ($rs['id_bitacora']."&&$contadorBN&&$contadorBNAnterior,");
                            }                            
                        ...
                        if(isset($contadorColor) && $contadorColor > 0){
                            if($existeContadorBN){
                                ...
                                $idBitacorasColor .= ($rs['id_bitacora']."&&$contadorBN&&$contadorBNAnterior&&$contadorColor&&$contadorColorAnterior,");
                            }                            
                            $existeContadorBN = true;
                        }
                        ...
                        if($cobrarRenta && $rs[$prefijos[$prefijo]."Renta"] > 0){
                            if($existeContadorBN){
                                ...
                                $idBitacorasRenta .= ($rs['id_bitacora']."&&$contadorBN&&$contadorBNAnterior&&$contadorColor&&$contadorColorAnterior,");
                            }                            
                            $existeContadorBN = true;
                        }
                        ...
                        if($MostrarUbicacion && $rs['Ubicacion'] != "")
                        { 
                        ...
                        if($particular){//Para los equipos en servicio particular, se calcula por fila los costos                            
                            ...
                            if($MostrarEquipos){
                                ...
                                if(!$mostrarRenta){//Si solo se muestra una fila por equipo ..
                                    ...
                                    $reporte->crearConceptoFactura(1, "Servicio", "COSTO POR EQUIPO SERIE ".$rs['NoSerie']."", 
                                            $totalParticular-$iva_particular, $totalParticular-$iva_particular, $NumeroFacturas, $NumeroConcepto);
                                    $fields = $reporte->getFields();
                                    $NumeroConcepto++;
                                    ...
                                    if($particular && $cobrarRenta){
                                        ...
                                        $reporte->crearConceptoFactura(1, $um, $des_aux,  $rs[$prefijos[$prefijo]."Renta"],  $rs[$prefijos[$prefijo]."Renta"], $NumeroFacturas, $NumeroConcepto);
                                        $fields = $reporte->getFields();
                                        $NumeroConcepto++;
                                    }
                                    if($cobrarExcedenteBN){                                    
                                        ...
                                        $reporte->crearConceptoFactura($aux, $um, $des_aux , $excedentesBN, $excedentesBN * $aux, $NumeroFacturas, $NumeroConcepto);
                                        $fields = $reporte->getFields();
                                        $NumeroConcepto++;
                                    }
                                    if($cobrarExcedenteColor && $color){
                                        ...
                                        $reporte->crearConceptoFactura($aux1, $um, $des_aux, 
                                                $excedentesColor, $excedentesColor * $aux1, $auxFactura, $auxConcepto);
                                        $fields = $reporte->getFields();
                                        $NumeroConcepto++;
                                    }
                                    if($cobrarProcesadasBN){
                                        ...
                                        $reporte->crearConceptoFactura($diferencia_bn, $um, $des_aux, 
                                                $procesadosBN, $procesadosBN * $diferencia_bn, $NumeroFacturas, $NumeroConcepto);
                                        $fields = $reporte->getFields();
                                        $NumeroConcepto++;
                                    }
                                    if($cobrarProcesadasColor && $color){
                                        ...
                                        $reporte->crearConceptoFactura($diferencia_color, $um, $des_aux, 
                                                $procesadosColor, $procesadosColor * $diferencia_color, $auxFactura, $auxConcepto);
                                        $fields = $reporte->getFields();
                                        $NumeroConcepto++;
                                    }
                                }
