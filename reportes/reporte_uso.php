<?php
session_start();
if (!isset($_SESSION['user']) || $_SESSION['user'] == "" || !isset($_POST['id_reporte'])) {
...
$consulta = "SELECT DISTINCT($order_by) FROM `k_reporteuso` WHERE IdReporte = $idReporte ORDER BY $order_by;";
...
$consulta = "SELECT SUM(Color) AS color, SUM(BN) AS bn FROM `k_reporteuso` WHERE IdReporte = ".$idReporte." AND (Fecha BETWEEN '".$_POST['fecha_inicial']."' AND '".$_POST['fecha_final']."');";
$query = $catalogo->obtenerLista($consulta);
$total = 0;
while($rs = mysql_fetch_array($query)){
                    ...
                    foreach ($secciones as $value) {
                        ...
                        echo "$('#container".$i."').highcharts({
                            ...
                            series: [{
                                    ...
                                    name: 'Porcentaje',
                                    data: [";
                        $consulta = "SELECT $order_by, (SELECT CASE WHEN tipo = 101 THEN 'Copia' WHEN tipo = 102 THEN 'Digitalización' ELSE 'Impresión' END) AS tipo_letra, tipo, SUM(Color) AS color, SUM(BN) AS bn FROM `k_reporteuso` WHERE IdReporte = ".$idReporte." "
                        . "AND $order_by LIKE '%$value' AND (Fecha BETWEEN '".$_POST['fecha_inicial']."' AND '".$_POST['fecha_final']."') GROUP BY $order_by, Tipo ORDER BY $order_by, Tipo;";
                        //echo $consulta;
                        $query = $catalogo->obtenerLista($consulta);
                        while($rs = mysql_fetch_array($query)){
