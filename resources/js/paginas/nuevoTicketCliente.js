...
$(document).ready(function() {
    ...
    if ($("#tipoUsuario").val() == "21"){
        var paginaExito = "mesa/alta_misticketphp.php";
    }else{
        if($("#permiso_tickets2").length && $("#permiso_tickets2").val()=="1"){
            var paginaExito = "mesa/lista_ticket_new.php";
        }else{
                ...
                $.post(controlador, {form: $(form).serialize(), "tabla": tabla}).done(function(data) {                    
                    if (data.toString().indexOf("Error:") === -1 && $.isNumeric(data.toString())) {
                        ...
                        $('#contenidos').load(paginaExito, {'idTicket':data}, function() {
                            $('#titulo').text("Mesa de ayuda > Consulta Ticket 2");
...
function MostrarTipoReporte(tipo) {
    loading("Cargando ...");
    $('#contenidos').load("mesa/alta_misticketphp.php", {"mostrarDatos": tipo}, function() {
        finished();
    });
}
...
function mostrarLocalidadTicket(cliente, serie) {
    ...
    $('#contenidos').load("mesa/alta_misticketphp.php", {"claveCliente": cliente, "mostrarDatos": tipoReporte}, function() {
    ...
    if (tipoReporte == "15") {
        $.post("WEB-INF/Controllers/Controler_BuscarDatosIncidencia.php", {"buscar": "BuxcarLocalidad", "localidad": localidad}).done(function(data) {
            ...
            if (data != "" && valores[0] != "" && permiso == "0") {
                ...
                $('#contenidos').load("mesa/alta_misticketphp.php", {"claveCliente": claveCliente, "mostrarDatos": tipoReporte, "claveLocalidad": localidad}, function() {
                    finished();
                });
            }
        });
                    ...
                    $("#dialog").dialog({
                        ...
                        buttons: {
                            ...
                            "Ir al ticket": function() {
                                loading("Cargando ...");
                                $('#contenidos').load("mesa/alta_misticketphp.php", {"idTicket": data, "area": "15", "detalle": "0"}, function() {
                                    finished();
                                });
                                $(this).dialog("close");
                            }
        ...
        $(function() {
            $("#dialog").dialog({
                ...
                buttons: {
                    ...
                    "Ir al ticket": function() {
                        loading("Cargando ...");
                        $('#contenidos').load("mesa/alta_misticketphp.php", {"idTicket": idTicket, "area": "1", "detalle": "0"}, function() {
                            finished();
                        });
                        $(this).dialog("close");
                    }
...
function buscarNoSerie(noSerie) {
    ...
    $('#contenidos').load("mesa/alta_misticketphp.php", {"noSerie": noSerie, "area": tipo}, function() {
        finished();
    });
}
...
function BuscarEquiposNumeroSerieLocalidad() {
    if (validar()) {
        ...
        loading("Cargando ...");
        $('#contenidos').load("mesa/alta_misticketphp.php", {"claveCliente": claveCliente, "mostrarDatos": tipoReporte, "claveLocalidad": localidad, "listSerie": getSelected()}, function() {
            finished();
        });
    } else {
