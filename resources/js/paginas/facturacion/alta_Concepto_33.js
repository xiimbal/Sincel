...
function GenerarFacturaSustitucion(id) {
    ...
    $("#contenidos_invisibles").load("WEB-INF/Controllers/facturacion/Controller_Verificar_Factura.php", {'idFactura': id}, function (data) {
        if (data.toString().indexOf("Error:") === -1) {//Si no hay caracteres especiales en los conceptos
            ...
            if ($("#fac_sust").length && $("#fac_sust").val() == "1") {
                fac_sust = 1;
                tipo = "Sustitucion de Factura";
                if ($("#IdFacturaNET").length) {
                    idFacturaNet = $("#IdFacturaNET").val();
                }
            }
                ...
                buttons: {
                    "Aceptar": function () {
                        if (confirm("¿ Esta seguro que desea generar la " + tipo + "?")) {
                            ...
                            //alert(leyenda);
                         loading("Procesando ...");
                                $.post("WEB-INF/Controllers/facturacion/Controller_nueva_Factura.php", {form: $("#altaFacturaform").serialize(), 'fac_sust': fac_sust}).done(function (data) {
                                if (data.toString().indexOf("Error:") === -1) {
                                    var id2 = 0;
                                    if (fac_sust === 1) {
                                        var ids = data.split(",");
                                    ...
                                    $.post("WEB-INF/Controllers/facturacion/Controler_Generar_Factura_33.php?id=" + id + "&id2=" + id2,
                                            {'leyenda': leyenda, 'fac_sust': fac_sust, 'IdFacturaNET': idFacturaNet, form: $("#altaFacturaform").serialize()}, function (data) {
                                        if (data.toString().indexOf("Error:") === -1) {
