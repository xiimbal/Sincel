...
function guardarParametros() {
    if ($(form).valid()) {
        var controlador = "WEB-INF/Controllers/facturacion/Controller_Parametros_Lectura.php";
        loading("Cargando ...");
...
function cargarclientes(origen, componente) {
    ...
    $("#" + componente).load("WEB-INF/Controllers/Ventas/Controller_select_clientes.php", {cliente: $("#" + origen).val()}, function(data) {
...
function agregarServicio() {
    $("#concepto_tabla").append("<tr><td><select name='select_con_adic_" + filas + "' id='select_con_adic_" + filas + "'></td>" +
            "<td><input type='text' name='text_con_adic_" + filas + "' id='text_con_adic_" + filas + "'/></td>" +
            "<td><input type='text' name='monto_con_adic_" + filas + "' id='monto_con_adic_" + filas + "'/></td>" +
            "<td><input type='text' name='unidad_con_adic_" + filas + "' id='unidad_con_adic_" + filas + "'/></td></tr>");
    ...
    $("#monto_con_adic_" + filas).rules('add', {
        required: true,
        messages: {
            required: "Ingresa el monto"
        }
    });
    $("#unidad_con_adic_" + filas).rules('add', {
        required: true,
        messages: {
            required: "Ingresa la unidad"
        }
...
function eliminarFila() {
    if (filas == 0) {
        ...
        $("#monto_con_adic_" + filas).rules('remove', 'required');
        $("#unidad_con_adic_" + filas).rules('remove', 'required');
