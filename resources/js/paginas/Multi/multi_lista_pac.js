...
function EliminarPAC(id) {
    var r = confirm("¿Esta seguro que desea eliminar el PAC?");
    if (r == true) {
        var dir = "WEB-INF/Controllers/Multi/Controller_PAC.php?id=" + id;
        $.post(dir, function(data) {
            if (data.toString().indexOf("Error:") === -1) {/*En caso de que no hay error*/
                cambiarContenidos("Multi/lista_pac.php", "PAC");
                $('#mensajes').html(data);
            } else {
                $('#mensajes').html(data);
            }
