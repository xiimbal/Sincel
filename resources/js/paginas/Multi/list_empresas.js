...
function eliminarEmpresa(id) {
    var r = confirm("¿Esta seguro que desea eliminar la empresa?");
    if (r == true) {
        var dir = "WEB-INF/Controllers/Multi/Controller_Empresa.php?id=" + id;
        $.post(dir, function(data) {
            if (data.toString().indexOf("Error:") === -1) {/*En caso de que no hay error*/
                cambiarContenidos("Multi/list_empresas.php", "Empresas");
                $('#mensajes').html(data);
            } else {
                $('#mensajes').html(data);
            }
