...
function agregarCategoria() {
    ...
    $("#t_datos_categorias").append("<li id='row_"+numero+"' class='list-group-item'>" +
            "<label class='m-2' for='categoria"+numero+"'>Categoria</label><div class='d-flex flex-row'>" +
            "<select id='categoria"+numero+"' name='categoria"+numero+"' class='m-2 custom-select' style='max-width: 200px;'></select>" +
            "<button class='btn btn-success m-2' title='Agregar otra categoría' onclick='agregarCategoria(); return false;'><span class='oi oi-plus'></span></button>" +
            "<button class='btn btn-danger m-2' title='Eliminar este periodo' onclick='borrarCategoria(" + numero + "); return false;'><span class='oi oi-x'></span></button>" +
            "</div></li>");   
...
function borrarCategoria(fila) {   
    ...
    // if(fila > contador){
        ...
        // }
        var row = 'row_' + fila;
        console.log(fila+" "+contador+" "+row);
    if (fila!=1) {//Si hay filas en la tabla        
        $("#" + row).remove();
        //$("#" + row).rules("remove");
