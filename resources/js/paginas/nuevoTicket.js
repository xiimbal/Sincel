...
$(document).ready(function () {
    ...
    if ($("#tipoUsuario").val() == "21") {
        var paginaExito = "mesa/alta_ticketphp.php";
    } else {
    ...
    $(form).validate({
        rules: {
            ...
            rdEquipoFalla: {validarSeleccion: true},
//            activar_0: {validarPedido: true},
    ...
    $(form).submit(function (event) {
        var tamanio = $("#tablaTamanio").val();
        var pedidoActivo = false;
        
        if($("#sltTipoReporte").val() == 15){
            
        for(var i = 0;i<tamanio;i++){
            if($("#activar_"+i).is(":checked")){
               pedidoActivo = true;
               break;
            }
        }
        ...
        if ($(form).valid() && pedidoActivo) {
            ...
            var tabla = "";
            if ($("#sltTipoReporte").val() != "15") {
                tabla = $('input:radio[name=rdEquipoFalla]:checked').val();
            } else if ($("#sltTipoReporte").val() == "15") {
                tabla = $("#tablaTamanio").val();
            }
            ...
            if (numSeleccion > 0) {
                if ($("#sltTipoReporte").val() == "15") {//Si el ticket es toner
                    //Validamos el rendimiento del toner

                    $.post("WEB-INF/Controllers/Ajax/altaTicketValida.php", {form: $(form).serialize(), "tabla": tabla, "valida_toner": 1})
                            .done(function (data2) {
                                if (data2 == "") {//no hay problema con el rendimiento
                                    mandarForm(form, controlador, tabla, paginaExito);
                                    finished();
                                } else {//El rendimiento no se cumple
                                    if ($("#txtPermisoRendimiento").val() == "1") {//permiso de rendimiento
                                        $("#dialog").html(data2 + "<br/>Desea continuar?");
                                        $(function () {
                                            $("#dialog").dialog({
                                                resizable: false, weight: 300, height: 300, modal: true, closeOnEscape: false,
                                        ...
                                        $("#dialog").html(data2 + "<br/>Si desea continuar solicite autorización, en caso contrario, cancele");
                                        $(function () {
                                            $("#dialog").dialog({resizable: true, weight: 300, height: 300, modal: true, closeOnEscape: false,
                                                open: function (event, ui) {
                                                    $(".ui-dialog-titlebar-close", ui.dialog).hide();
                                                },
                                                buttons: {
                                                    ...
                                                    "Solicitar Autorización": function () {
                                                        $(this).dialog("close");
                                                        mandarForm(form, controlador + "?pendiente_autorizar=1", tabla, paginaExito);
                                                        finished();
                                                    }
                                                }
                                            });
                        ...
                        alert("Error al intentar validar tóner" + data);
                    });
                } else {
                    mandarForm(form, controlador, tabla, paginaExito);
                }
            } else {
                $("#SeleccionPedido").html("El ticket no tiene un pedido, para continuar seleccione minimo uno");
                $(function () {
                    $("#SeleccionPedido").dialog({
                        resizable: false,
                        height: 200,
                        modal: true,
                        closeOnEscape: false,
            ...
            if(!pedidoActivo){
                alert("Debe activar al menos un pedido");
            }
        }
    });
...
function mandarForm(form, controlador, tabla, paginaExito) {

    $.post(controlador, {form: $(form).serialize(), "tabla": tabla}).done(function (data) {
        if (data.toString().indexOf("Error:") === -1) {
            var str = data.toString();
            $('#mensajes').html(str.substring(0, str.length - 6) + "<br/>El ticket <b>" + str.substring((str.length) - 6, str.length) + "</b> se registró correctamente");
...
function mostrarLocalidadTicket(cliente, serie) {
    ...
    $('#contenidos').load("mesa/alta_ticketphp.php", {"claveCliente": cliente, "mostrarDatos": tipoReporte}, function () {
    ...
    if (tipoReporte == "15") {
        $.post("WEB-INF/Controllers/Controler_BuscarDatosIncidencia.php", {"buscar": "BuxcarLocalidad", "localidad": localidad}).done(function (data) {
            ...
            if (data != "" && valores[0] != "" && permiso == "0") {
                ...
                $('#contenidos').load("mesa/alta_ticketphp.php", {"claveCliente": claveCliente, "mostrarDatos": tipoReporte, "claveLocalidad": localidad}, function () {
                    finished();
                });
            }
        });
    ...
    if ($("#activar_" + fila).is(':checked')) {
        ...
        jQuery.validator.addMethod('validarContadorNegroTonerMayor', function (value) {
            if (parseInt($("#txtContadorNegro_" + fila).val()) > parseInt($("#txtContadorNegroAnterior_" + fila).val()) + 100000) {
                $('#errorNegro' + fila).text(' * El contador no debe superar mas de 100,000 al anterior');
                return true;
            } else {
                $('#errorNegro' + fila).text('');
                return true;
            }
        }, "");
        ...
        if (tipoColor == "1") {
            ...
            jQuery.validator.addMethod('validarContadorColorTonerMayor', function (value) {
                if (parseInt($("#txtContadorColor_" + fila).val()) > parseInt($("#txtContadorColorAnterior_" + fila).val()) + 100000) {
                    $('#errorColor' + fila).text(' * El contador no debe superar mas de 100,000 al anterior');
                    return true;
                } else {
                    $('#errorColor' + fila).text('');
                    return true;
                }
            }, "");
                    ...
                    $("#dialog").dialog({
                        ...
                        buttons: {
                            ...
                            "Ir al ticket": function () {
                                loading("Cargando ...");
                                $('#contenidos').load("mesa/alta_ticketphp.php", {"idTicket": data, "area": "15", "detalle": "0"}, function () {
                                    finished();
                                });
                                $(this).dialog("close");
                            }
...
function NostrarInputs(fila, noSerie) {
    if ($("#activar_" + fila).is(':checked')) {
        $.post("WEB-INF/Controllers/Controler_BuscarDatosIncidencia.php", {"buscar": "LecturaNoSerie", "NoSerie": noSerie, "tipo": "15"}).done(function (data) {
            ...
            //Oculto los contadores de ultima lectura y ultimo cambio de tóner
            $("#lecturaCambioToner_" + fila).hide();
            $("#ultimaLecturaCorte_" + fila).hide();
            $("#lecturaCambioTonerCian_" + fila).hide();
            $("#ultimaLecturaCorteCian_" + fila).hide();
            $("#lecturaCambioTonerMagenta_" + fila).hide();
            $("#ultimaLecturaCorteMagenta_" + fila).hide();
            $("#lecturaCambioTonerAmarillo_" + fila).hide();
            $("#ultimaLecturaCorteAmarillo_" + fila).hide();
            //Le asignamos el texto correspondiente
            if (valor[7] != null && valor[7].trim() != "") {
                $("#ultimaLecturaCorte_" + fila).text("Lectura Corte:" + valor[7]);
            } else {
                $("#ultimaLecturaCorte_" + fila).text("No hay lectura corte");
            }
            if (valor[8] != null && valor[8].trim() != "") {
                $("#ultimaLecturaCorteCian_" + fila).text("Lectura Corte:" + valor[8]);
                $("#ultimaLecturaCorteMagenta_" + fila).text("Lectura Corte:" + valor[8]);
                $("#ultimaLecturaCorteAmarillo_" + fila).text("Lectura Corte:" + valor[8]);
            } else {
                $("#ultimaLecturaCorteCian_" + fila).text("No hay lectura corte");
                $("#ultimaLecturaCorteMagenta_" + fila).text("No hay lectura corte");
                $("#ultimaLecturaCorteAmarillo_" + fila).text("No hay lectura corte");
            }
            if (valor[9] != null && valor[9].trim() != "") {
                $("#lecturaCambioToner_" + fila).text("Lectura cambio tóner:" + valor[9]);
            } else {
                $("#lecturaCambioToner_" + fila).text("No hay lectura cambio tóner");
            }
            if (valor[10] != null && valor[10].trim() != "") {
                $("#lecturaCambioTonerCian_" + fila).text("Lectura cambio tóner:" + valor[10]);
                $("#lecturaCambioTonerMagenta_" + fila).text("Lectura cambio tóner:" + valor[10]);
                $("#lecturaCambioTonerAmarillo_" + fila).text("Lectura cambio tóner:" + valor[10]);
            } else {
                $("#lecturaCambioTonerCian_" + fila).text("No hay lectura cambio tóner");
                $("#lecturaCambioTonerMagenta_" + fila).text("No hay lectura cambio tóner");
                $("#lecturaCambioTonerAmarillo_" + fila).text("No hay lectura cambio tóner");
            }
        });
    } else {
        ...
        $(function () {
            $("#dialog").dialog({
                ...
                buttons: {
                    ...
                    "Ir al ticket": function () {
                        loading("Cargando ...");
                        $('#contenidos').load("mesa/alta_ticketphp.php", {"idTicket": idTicket, "area": "1", "detalle": "0"}, function () {
                            finished();
                        });
                        $(this).dialog("close");
                    }
...
function buscarNoSerie(noSerie) {
    ...
    $('#contenidos').load("mesa/alta_ticketphp.php", {"noSerie": noSerie, "area": tipo}, function () {
        finished();
    });
}
    ...
    $.post("WEB-INF/Controllers/Controler_BuscarDatosIncidencia.php", {"buscar": "BuscarDatosPorSerieVentaDirecta", "NoSerie": serie}).done(function (data) {
        ...
        if (datos[0] == "1" || datos[0] == "-1") {
            ...
            $.post("WEB-INF/Controllers/Controler_BuscarDatosIncidencia.php", {"buscar": "BuscarDatosPorSerieFalla", "NoSerie": serie}).done(function (data) {
                ...
                console.log(datos);
                if (idTicket != "") {
                    $("#dialog").html("Existe un ticket abierto para el No. de Serie proporcionado: ticket <b>" + idTicket + "</b>");
                    $(function () {
                        $("#dialog").dialog({
                            resizable: false,
                            height: 200,
...
function BuscarEquiposNumeroSerieLocalidad() {
    if (validar()) {
        ...
        loading("Cargando ...");
        $('#contenidos').load("mesa/alta_ticketphp.php", {"claveCliente": claveCliente, "mostrarDatos": tipoReporte, "claveLocalidad": localidad, "listSerie": getSelected()}, function () {
            finished();
        });
    } else {
...
function validarRendimiento(cktoner, color, fila, cian, magenta, amarillo) {
    ...
    if (color === 0) {//toner negro
        contadorAnterior = parseInt($("#txtContadorNegroAnterior_" + fila).val());
        contadorActual = parseInt($("#txtContadorNegro_" + fila).val());
        if (contadorAnterior !== "" && (contadorActual < contadorAnterior)) {
            $("input[name=" + cktoner + "]").attr('checked', false);
            $("#error_toner_compatible_negro" + fila).html(" * La lectura actual no puede ser menor a la anterior ");
            return false;
        }
        ...
        $("#ultimaLecturaCorte_" + fila).show();
        $("#lecturaCambioToner_" + fila).show();
    } else {//toner color
        contadorAnterior = parseInt($("#txtContadorColorAnterior_" + fila).val());
        contadorActual = parseInt($("#txtContadorColor_" + fila).val());

        if (contadorAnterior != "" && contadorActual < contadorAnterior) {
            $("input[name=" + cktoner + "]").attr('checked', false);
            $("#error_toner_compatible_cian" + fila).html(" * La lectura actual no puede ser menor a la anterior");
            return false;
        }
        ...
        if (cian == "1") {//toner cian
            ...
            $("#ultimaLecturaCorteCian_" + fila).show();
            $("#lecturaCambioTonerCian_" + fila).show();
            ...
            $("#ultimaLecturaCorteMagenta_" + fila).show();
            $("#lecturaCambioTonerMagenta_" + fila).show();
            ...
            $("#ultimaLecturaCorteAmarillo_" + fila).show();
            $("#lecturaCambioTonerAmarillo_" + fila).show();
        }
    }
    ...
    if ($("#" + cktoner).is(":checked")) {
        if (contadorAnterior !== "" && !isNan(contadorAnterior)) {
            if (contadorActual !== "") {
