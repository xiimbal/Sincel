...
if (isset($_POST['form'])) {
    ...
    $cxc = true;
    $cabeceras = Array("Folio", "Fecha", "Cliente", "Pagado", "Saldo", "Total", "PDF","Pagado", "Pagos");
    ...
    $pagado = 0;
    ?>
    <script type="text/javascript" language="javascript" src="resources/js/paginas/remision/lista_busqueda_remisiones_pakal.js"></script>

    <div class="table-responsive">
        <table id="treportfact" class="table">
            ...
            <tbody>
                ...
                while ($rs = mysql_fetch_array($query)) {
                    ...
                    echo "<tr>
                            <td  $color>" .$rs['Folio'] . "</td>
                            <td  $color>" . $rs['FechaFacturacion'] . "</td>";
                    /*if(!$cxc){
                        echo "<td $color>" . $rs['ClaveCliente'] . "</td>";
                    }else{
                        echo "<td $color>" . $rs['ejecutivo'] . "</td>";
                    }*/
                    echo "<td $color>" . $rs['NombreReceptor'] . "</td>";
                    //echo "<td $color>" . $rs['RFCEmisor'] . "</td>";
                    if(!$cxc){
                        echo "<td $color>$" . $rs['subtotal'] . "</td>";
                        echo "<td $color>$" . $rs['importe'] . "</td>";
                    }else{
                        echo "<td $color>$" . number_format($rs['pagado'],2) . "</td>";
                        echo "<td $color>$" . number_format((float)$rs['TotalSinFormato'] - (float)($rs['pagado']),2) . "</td>";
                    }
                    echo "<td $color>$" . $rs['Total'] . "</td>";
                    echo "<td>
                            <a href='WEB-INF/Controllers/facturacion/Controller_PDF_Factura.pakal.php?id=" . $rs['IdFactura'] . "' target='_blank'>
                                <img src='resources/images/pdf_descarga.png' title='PDF Factura' style='width: 32px; height: 32px;'/>
                            </a>
                        </td>";                 
                    //echo "<td $color>" . $rs['RFCReceptor'] . "</td>";
                    if ($rs['PagadoSiNo'] != 'No') {
                        echo "<td $color>";
                        if(!$cxc){
                            echo $rs['PagadoSiNo'];
                    ...
                    echo "<td $color>".str_replace('()', '', $rs['pagos'])."</td>";
                    echo "</tr>";
                }
                ?>
            </tbody>
