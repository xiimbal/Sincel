    ...
    <body>
        ...
        <div class="principal">
            ...
            if (isset($_POST['id'])) {
                ...
                $consulta = "SELECT c.ClaveCliente, cc.ClaveCentroCosto 
                    FROM `k_minialmacenlocalidad` AS kma
                    LEFT JOIN c_almacen AS a ON a.id_almacen = kma.IdAlmacen
                    LEFT JOIN c_centrocosto AS cc ON cc.ClaveCentroCosto = kma.ClaveCentroCosto
                    LEFT JOIN c_cliente AS c ON c.ClaveCliente = cc.ClaveCliente
                    WHERE a.id_almacen = $idAlmacen
                    GROUP BY cc.ClaveCentroCosto
                    ORDER BY c.ClaveCliente, cc.ClaveCentroCosto;";
                $result = $catalogo->obtenerLista($consulta);
                while($rs = mysql_fetch_array($result)){
                    if(!in_array($rs['ClaveCliente'], $cliente)){
                        array_push($cliente, $rs['ClaveCliente']);
                    }
                    $localidades[$rs['ClaveCentroCosto']] = $rs['ClaveCliente'];
                }
                ...
                ?>
                    <table  id="TablaClienteFila" style="width:100%">                       
                        <tr>
                            ...
                            <td>
                                <select class="cliente" id="cliente_0" name="cliente_0" style="width: 190px" onchange="mostraLocalidadAjaxCliente('<?php echo "0" ?>');" >
                            ...
                            <td>
                                <select id="localidad0" name="localidad0[]" style="width: 190px" multiple="multiple" class="localidad">
                                    <?php
                                    if ($cliente[0] != "") {                                        
                                        $query1 = $catalogo->obtenerLista("SELECT * FROM c_centrocosto cc WHERE cc.ClaveCliente='$cliente[0]';");
                                        while ($rs = mysql_fetch_array($query1)) {
                                            $s1 = "";                                            
                                            foreach ($localidades as $key => $value) {
                                                if ($value == $cliente[0] && $rs['ClaveCentroCosto'] == $key) {
                                                    $s1 = "selected";
                                                }                                                
                                            }                                                                                            
                                ...
                                <td>
                                    <select id="localidad_<?php echo $ct; ?>" name="localidad_<?php echo $ct; ?>[]" style="width: 190px" multiple="multiple" class="localidad">
                                        ...
                                        if ($cliente[$ct] != "") {
                                            ...
                                            $query1 = $catalogo->obtenerLista("SELECT * FROM c_centrocosto cc WHERE cc.ClaveCliente='$cliente[$ct]'");
                                            while ($rs = mysql_fetch_array($query1)) {
                                                $s1 = "";
                                                foreach ($localidades as $key => $value) {
                                                    if ($value == $cliente[$ct] && $rs['ClaveCentroCosto'] == $key) {
                                                        $s1 = "selected";
                                                    }                                                    
                                                }  
                                                echo "<option value='" . $rs['ClaveCentroCosto'] . "' $s1>" . $rs['Nombre'] . "</option>";
